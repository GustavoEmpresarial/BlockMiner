/**
 * Boosts — reward duration, entitlement expiry, and activation rules.
 * Mirrors legacy/tests/power-boost/powerBoost.service.test.mjs.
 */
...
import {
  ...
  todayKeyUTC,
} from "../../server/modules/boosts/boosts.service.ts";

test("scenario 1: without boost, faucet reward expires in exactly 24h", async () => {
  const earnedAt = new Date("2026-07-03T08:00:00.000Z");
  const tx = { dailyPowerBoost: { findUnique: async () => null } };
...
test("scenario 2: with boost, faucet reward expires in exactly 7 days", async () => {
  const earnedAt = new Date("2026-07-03T08:00:00.000Z");
  const tx = { dailyPowerBoost: { findUnique: async () => ({ id: 99 }) } };
  const ttlMs = await getRewardDurationMsTx(tx, 1, "faucet");
  const expiresAt = computeRewardExpiresAt(earnedAt, ttlMs);
...
test("scenario 3: with boost, YouTube uses same 7-day TTL", async () => {
...
test("scenario 4: after boost entitlement ends, new rewards use 24h", async () => {
...
test("scenario 6: re-activation same UTC day is blocked (replace, not stack)", async () => {
  ...
  const tx = { dailyPowerBoost: { findUnique: async () => ({ id: 5 }) } };
  assert.equal(await hasActiveBoostTx(tx, 9, dayKey), true);
  assert.equal(BOOST_COST_POL, 0.01);
});

test("boost entitlement expires at next UTC midnight", () => {
