...
echo "=========================================================="
echo "   🛡️ KALI CONTAINER PENTEST AUDIT: ADMIN FAUCET MODULE   "
echo "=========================================================="
...
# Start local faucet test server on port 5116
echo "[*] Starting local faucet test server on port ${PORT}..."
PORT=${PORT} npx tsx tests/security/local-faucet-test-server.mjs &
SERVER_PID=$!
...
for i in {1..20}; do
  if curl -s "${TARGET}/api/admin/faucet/config" >/dev/null 2>&1; then
...
docker run --rm --network host -i "$IMAGE_NAME" python3 - "${TARGET}" "${ADMIN_TOKEN}" < tests/security/kali_faucet_pentest.py
