...
echo "=========================================================="
echo "   🛡️ KALI CONTAINER PENTEST AUDIT: ADMIN BANNERS MODULE  "
echo "=========================================================="
...
# Start local banners test server on port 5106
echo "[*] Starting local banners test server on port ${PORT}..."
PORT=${PORT} npx tsx tests/security/local-banners-test-server.mjs &
SERVER_PID=$!
...
docker run --rm --network host -i "$IMAGE_NAME" python3 - "${TARGET}" "${ADMIN_TOKEN}" < tests/security/kali_banners_pentest.py
