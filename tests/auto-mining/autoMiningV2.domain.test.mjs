...
} from "../../server/modules/auto-mining/auto-mining.domain.ts";
...
test("canGrantDaily enforces 1000 H/s combined ceiling", () => {
  assert.equal(canGrantDaily(DAILY_LIMIT_HASH - NORMAL_HASH_PER_CYCLE, NORMAL_HASH_PER_CYCLE, DAILY_LIMIT_HASH), true);
  assert.equal(canGrantDaily(DAILY_LIMIT_HASH - NORMAL_HASH_PER_CYCLE + 1, NORMAL_HASH_PER_CYCLE, DAILY_LIMIT_HASH), false);
...
test("assertValidMiningMode accepts NORMAL only — Turbo was retired", () => {
  assert.equal(assertValidMiningMode(MINING_MODES.NORMAL), MINING_MODES.NORMAL);
  assert.throws(() => assertValidMiningMode(MINING_MODES.TURBO), /Invalid mining mode/);
