...
import { getDailyTasksDashboard, postDailyTaskClaim } from './tasks.api';
...
export function useDailyTasksDashboard() {
  ...
  const load = useCallback(async () => {
    ...
    setLoadFailed(false);
    try {
      const res = await getDailyTasksDashboard();
      if (res.data?.ok) {
        setData(res.data);
        setLoadFailed(false);
      } else {
      ...
      if (axios.isAxiosError(e)) {
        ...
        if (status === 401) toast.error(t('dailyTasks.errors.unauthorized'));
        else if (status != null && status >= 500) toast.error(t('dailyTasks.errors.server'));
        else if (e.response) toast.error(t('dailyTasks.errors.load_failed'));
        else toast.error(t('dailyTasks.errors.network'));
      } else {
  ...
  const tasks: DailyTaskRow[] = useMemo(() => data?.tasks ?? [], [data?.tasks]);
  ...
  const claim = useCallback(
    async (taskId: number) => {
      try {
        ...
        const res = await postDailyTaskClaim(taskId);
        if (res.data?.ok) {
  ...
  return { t, loading, loadFailed, tasks, tasksByCadence, nextResetLabel, claimingId, load, claim };
}
