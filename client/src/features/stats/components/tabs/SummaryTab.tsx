...
import {
  ...
  percentOfTotal,
  resolveTotals,
} from '../../utils/earningsAnalytics';
import { EARNINGS_CATEGORY_KEYS } from '../../lib/stats.config';
import type { StatsDashboardContext } from '../../lib/stats.types';
import { formatPolAmount } from '../../lib/stats.earnings.api';
...
function SummaryTab({
  ...
  const powerMeta = earnings?.powerMeta;
  const expiryRows = (overview?.nextExpirations ?? []).slice(0, 5);
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4">
        ...
        <StatCard
          ...
          value={powerMeta?.activeBoosts ?? '—'}
          sub={
            <span className="inline-flex items-center gap-1 text-sky-400">
              <TrendingUp className="w-3 h-3" />+{formatHashrate(powerMeta?.powerGained24h)} {t('powerStats.dashboard.last_24h')}
            </span>
          }
          icon={Zap}
          accent="text-amber-400"
      ...
      <section className="rounded-2xl border border-slate-800 bg-slate-900/40 p-6">
        ...
        <Suspense fallback={<div className="h-24 animate-pulse rounded-xl bg-slate-800/40" />}>
          <ExpiryProgressList rows={expiryRows} />
        </Suspense>
      </section>
    </div>
  );
