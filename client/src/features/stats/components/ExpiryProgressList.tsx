...
// eslint-disable-next-line react-refresh/only-export-components -- pure helper colocated with its sole consumer below.
...
/**
 * Pure core of {@link progressPercent}: the elapsed-percentage of a temporary
 * boost's window at a fixed `now`. Extracted so the progress-bar math is unit-testable
 * without depending on the live clock.
 */
// eslint-disable-next-line react-refresh/only-export-components -- unit-testable pure core, colocated with its sole consumer below.
export function progressPercentAt(
  ...
  return (
    <ul className="space-y-3">
      {sorted.map((row, idx) => {
        ...
        // eslint-disable-next-line react-hooks/purity -- reads the wall clock to compute time remaining; list re-renders on an interval.
