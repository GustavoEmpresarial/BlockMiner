/**
 * BlockMiner verify scenes — post-match validation + result debrief.
 */
...
import type { GameVerifyRecord } from "../lib/finish/gameVerifyStorage";
  ...
  return (
    <>
      ...
      <div className="mt-6 flex flex-col items-center gap-3">
        <AdRotator ads={POWER_STATS_ADS_300} size="300x250" className="!my-0" />
      </div>
    </>
  );
}
...
export function ResultScene({
  ...
  return (
    <>
          ...
          <div className="relative flex flex-col items-center text-center">
            ...
            {isResultSuccess ? (
              ...
              <p className="mx-auto mt-4 max-w-md text-sm leading-relaxed text-slate-400">
                {(() => {
                  const reasonMessage =
                    typeof resolution?.reasonMessage === "string" && resolution.reasonMessage.trim()
                      ? resolution.reasonMessage.trim()
                      : null;
                  if (reasonMessage) return reasonMessage;
                  const reasonKey =
                    typeof resolution?.reasonKey === "string" && resolution.reasonKey.trim()
                      ? resolution.reasonKey.trim()
                      : null;
                  if (reasonKey) {
                    const keyed = t(`minerGames.game_finish.${reasonKey}`);
                    if (keyed && keyed !== `minerGames.game_finish.${reasonKey}`) return keyed;
                  }
                  return isRejected ? t("gameFlow.rejected_subtitle") : t("gameFlow.failure_subtitle");
                })()}
              </p>
            )}
          </div>
      ...
      <div className="mt-6 flex flex-col items-center gap-3">
        <AdRotator ads={LEADERBOARD_ADS} size="728x90" className="!my-0" />
      </div>
    </>
  );
}
