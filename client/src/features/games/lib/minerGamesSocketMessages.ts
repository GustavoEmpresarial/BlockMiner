...
export function translateGameFinishedFailure(
  ...
  // Prefer server message (includes no-reward / no-tournament copy for security denies).
  if (typeof data?.message === 'string' && data.message.trim()) return data.message.trim();
  if (data?.messageCode && typeof data.messageCode === 'string') {
    const key = `minerGames.game_finish.${data.messageCode}`;
    const keyed = t(key);
    if (keyed && keyed !== key) return keyed;
  }
  return '';
}
