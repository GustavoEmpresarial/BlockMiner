...
import { readAxiosResponseMessage } from '../../checkin/lib/httpPollingGuard';
import { useTranslation } from 'react-i18next';
import { TaxPayCurrencyPicker } from './TaxPayCurrencyPicker';
import {
  formatTaxPayAmount,
...
/** Map legacy <Trans> strings with <strong> tags to styled spans. */
function TransStrong({ text, strongClassName = 'font-black' }: { text: string; strongClassName?: string }): ReactNode {
  const parts = text.split(/(<strong>[\s\S]*?<\/strong>)/g);
  return parts.map((part, i) => {
    const m = /^<strong>([\s\S]*)<\/strong>$/.exec(part);
    if (m) {
      return (
...
export default function EnergyTaxSection() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setSummary(r.data);
      if (r.data.todayPayQuotes) {
        setPayCurrency((prev) => {
          const q = r.data.todayPayQuotes![prev];
          if (q?.affordable) return prev;
          return pickDefaultTaxPayCurrency(r.data.todayPayQuotes);
        });
      }
    } catch {
  ...
  const payToday = async () => {
    ...
    try {
      ...
      const msg = readAxiosResponseMessage(e, t('taxes.energy_tax.toast_pay_error_default'));
      toast.error(msg);
    } finally {
      setPaying(false);
    }
  ...
  const payQuote = summary.todayPayQuotes?.[payCurrency];
  const payBlockedByBalance = !todayExempt && !!payQuote && !payQuote.affordable;
  ...
  return (
    <div className="w-full space-y-6">
        ...
        <div className="rounded-2xl border-2 border-sky-500/30 bg-gradient-to-br from-sky-500/15 via-slate-900/80 to-slate-900 p-5 sm:p-7 flex items-start gap-4">
          ...
          <div className="flex-1">
            ...
            <p className="text-sm sm:text-base text-white mt-1">
              <TransStrong text={t('taxes.energy_tax.starts_in_body', { datetime: startsAtDatetime })} />
            </p>
            {startsInLabel && (
              <p className="text-xs sm:text-sm text-sky-200/80 mt-2 font-mono">
                <TransStrong
                  text={t('taxes.energy_tax.starts_in_countdown', { remaining: startsInLabel })}
                  strongClassName="font-black text-sky-200"
                />
              </p>
            )}
          </div>
      ...
      <div className="rounded-2xl border border-amber-500/25 bg-slate-900/60 p-5 sm:p-7">
        ...
        {!notYetActive && !summary.todayPaid && summary.yesterdayRewards > 0 && !todayExempt && (
          <div className="mb-4">
        ...
        <button
          ...
          disabled={todayBlocked || paying || payBlockedByBalance}
          className={`w-full py-5 font-black text-base rounded-xl transition-all uppercase tracking-widest flex items-center justify-center gap-2 ${
            todayBlocked || payBlockedByBalance
              ...
              : todayExempt
                ...
                : summary.yesterdayRewards <= 0
                  ...
                  : payBlockedByBalance
                    ...
                    : t('taxes.energy_tax.pay_button_pay', {
                        amount: formatTaxPayAmount(payQuote?.amount ?? summary.todayDailyCharge, payCurrency),
                      })}
        </button>
