...
export type WalletBalanceResponse = {
  ok: boolean;
  /** POL available balance (legacy field name). */
  balance?: number;
  /** Some rewrite payloads used this; client falls back if `balance` is absent. */
  polBalance?: number;
  blkBalance?: number;
  blkLocked?: number;
  shibBalance?: number;
...
export type SwapAsset = 'POL' | 'SHIB';

export type SwapBalancesResponse = {
  ok: boolean;
  balances?: Record<string, number>;
  prices?: Partial<Record<SwapAsset, number>>;
};
