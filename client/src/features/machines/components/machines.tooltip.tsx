 ...
 */
function computeTooltipPosition(anchor: DOMRect, tooltipWidth: number, tooltipHeight: number): { top: number; left: number } {
  ...
  left = Math.min(Math.max(left, VIEWPORT_MARGIN), window.innerWidth - VIEWPORT_MARGIN - tooltipWidth);
  top = Math.min(Math.max(top, VIEWPORT_MARGIN), window.innerHeight - VIEWPORT_MARGIN - tooltipHeight);

  return { top, left };
}

type RackMachineTooltipPortalProps = {
  open: boolean;
  anchorEl: HTMLElement | null;
  displayName: string;
  /** Formatted hashrate (includes unit). */
  hashrateStr: string;
  /** Number of rack slots occupied (e.g. 1 or 2). */
  slotSize: number;
};

/**
 * Portal-mounted tooltip for rack machine slots. Escapes ancestor overflow/transform clipping.
 * Marked aria-hidden: screen readers use the slot button's aria-label instead.
 */
export function RackMachineTooltipPortal({ open, anchorEl, displayName, hashrateStr, slotSize }: RackMachineTooltipPortalProps) {
  ...
  return createPortal(
    <div
      ...
      aria-hidden="true"
    >
      <p className="text-[9px] font-bold uppercase tracking-wide text-gray-500">{t('machines.tooltip_label')}</p>
      ...
      <p className="mt-0.5 text-[10px] font-bold uppercase tracking-wide text-primary">
        {t('machines.tooltip_power', { hashrate: hashrateStr })}
      </p>
      <p className="mt-0.5 text-[10px] font-bold uppercase tracking-wide text-gray-300">
        {t(slotSize === 1 ? 'machines.tooltip_slot_one' : 'machines.tooltip_slot_other', { count: slotSize })}
      </p>
    </div>,
    document.body,
  );
