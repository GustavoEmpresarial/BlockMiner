...
import { sanitizeMachineImageSrc } from './lib/ranking.utils';
...
interface RankingRoomApiResponse {
  ok?: boolean;
...
interface RankingEntry {
  ...
  gameHashRate?: number | string;
}

interface RankingListApiResponse {
  ok?: boolean;
...
function MiniRacks({ username, navigate }: { username: string; navigate: NavigateFunction }) {
  ...
  useEffect(() => {
    ...
    api.get<RankingRoomApiResponse>(`/ranking/room/${username}`)
      .then(r => { if (r.data.ok && r.data.user) setData(r.data.user); })
      .catch(() => {})
    ...
    <div className="px-6 pb-6 pt-2 space-y-4 bg-slate-950/50">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {Array.from({ length: RACKS_COUNT }).map((_, i) => {
          const rackIndex = i + 1;
          const rackName = racks[String(rackIndex)] || `Rack ${rackIndex}`;
