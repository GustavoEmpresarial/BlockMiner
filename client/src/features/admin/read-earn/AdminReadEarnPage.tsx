...
function fromLocalInput(s: string | undefined | null): string {
...
export default function AdminReadEarn() {
  ...
  const [rows, setRows] = useState<ReadEarnCampaignRow[]>([]);
  const [miners, setMiners] = useState<AdminMinerRow[]>([]);
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [cRes, mRes] = await Promise.all([
        api.get<CampaignsListResponse>('/admin/read-earn/campaigns'),
        api.get<MinersListResponse>('/admin/miners')
      ]);
      if (cRes.data?.ok) setRows(cRes.data.campaigns || []);
      else toast.error(adminReadEarnErrMessage({ data: cRes.data }, t, 'adminReadEarn.load_error'));
      if (mRes.data?.ok) setMiners(mRes.data.miners || []);
    } catch (e: unknown) {
      toast.error(adminReadEarnErrMessage(e, t, 'adminReadEarn.load_error'));
    } finally {
      setLoading(false);
    }
  }, [t]);
  ...
  const setField = <K extends keyof ReadEarnFormState>(k: K, v: ReadEarnFormState[K]) =>
    setForm((f) => ({ ...f, [k]: v }));
  ...
  const startEdit = (row: ReadEarnCampaignRow) => {
    setEditingId(row.id);
    setForm({
      title: row.title,
  ...
  const loadRedemptions = async (c: ReadEarnCampaignRow) => {
    ...
    try {
      ...
      if (res.data?.ok) {
        ...
        toast.error(adminReadEarnErrMessage({ data: res.data }, t, 'adminReadEarn.redemptions_load_error'));
      }
    } catch (e: unknown) {
      toast.error(adminReadEarnErrMessage(e, t, 'adminReadEarn.redemptions_load_error'));
    } finally {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (editingId === 'new') {
        ...
        const res = await api.post<MutateCampaignResponse>('/admin/read-earn/campaigns', body);
        if (!res.data?.ok) {
          toast.error(adminReadEarnErrMessage({ data: res.data }, t, 'adminReadEarn.toast_save_error'));
          return;
        }
        ...
        const res = await api.put<MutateCampaignResponse>(`/admin/read-earn/campaigns/${editingId}`, body);
        if (!res.data?.ok) {
          toast.error(adminReadEarnErrMessage({ data: res.data }, t, 'adminReadEarn.toast_save_error'));
          return;
        }
      ...
      toast.error(adminReadEarnErrMessage(e, t, 'adminReadEarn.toast_save_error'));
    } finally {
      setSaving(false);
    }
  };
            ...
            {form.rewardType === 'hashrate' && (
              <label className="block space-y-1">
                ...
                <input
                  ...
                  onChange={(e) => setField('hashrateValidityDays', Number(e.target.value))}
                />
              </label>
            )}
            <label className="block space-y-1">
