...
export default function CreditPolModal({ open, ticketId, playerLabel, onClose, onCredited }: Props) {
  ...
  const handleSubmit = async () => {
    ...
    if (!Number.isFinite(parsed) || parsed <= 0) {
      toast.error(t('adminSupport.creditPol.invalid_amount'));
      return;
    }
    if (parsed > 1000) {
      toast.error(t('adminSupport.creditPol.amount_too_high'));
      return;
    }
    if (reason.trim().length < 3) {
      toast.error(t('adminSupport.creditPol.reason_required'));
      return;
    }
    setSubmitting(true);
    try {
      ...
      toast.error(resolveApiErrorMessage(err, t('adminSupport.creditPol.error')));
    } finally {
      setSubmitting(false);
    }
  };
  ...
  return (
    ...
    >
      ...
      >
        ...
        {playerLabel ? (
          <p className="mb-3 text-xs text-slate-400">{t('adminSupport.creditPol.target', { player: playerLabel })}</p>
        ) : null}

        <label className="mb-3 block">
          <span className="mb-1 block text-[10px] font-bold uppercase tracking-widest text-slate-400">
            {t('adminSupport.creditPol.amount_label')}
          </span>
          <input
            type="number"
            inputMode="decimal"
        ...
        <label className="mb-4 block">
          <span className="mb-1 block text-[10px] font-bold uppercase tracking-widest text-slate-400">
            {t('adminSupport.creditPol.reason_label')}
          </span>
          <textarea
            value={reason}
            onChange={(e) => setReason(e.target.value)}
            rows={3}
            placeholder={t('adminSupport.creditPol.reason_placeholder')}
        ...
        <p className="mb-4 rounded-lg border border-amber-900/40 bg-amber-950/20 px-3 py-2 text-[11px] leading-relaxed text-amber-200/80">
          {t('adminSupport.creditPol.warning')}
        </p>
        ...
        <div className="flex justify-end gap-2">
          ...
          >
            {t('adminSupport.creditPol.cancel')}
          </button>
          <button
            type="button"
          ...
          >
            ...
            {t('adminSupport.creditPol.confirm')}
          </button>
        </div>
      </div>
    </div>
