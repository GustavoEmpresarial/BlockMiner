...
import type { AdminMiniPassSeasonListRow } from '../lib/admin.types';

type MiniPassSeasonsResponse = {
  ok?: boolean;
  seasons?: AdminMiniPassSeasonListRow[];
};

export default function AdminMiniPass() {
  const { t } = useTranslation();
  const navigate = useNavigate();
  const [rows, setRows] = useState<AdminMiniPassSeasonListRow[]>([]);
  const [loading, setLoading] = useState(true);

  const load = useCallback(async () => {
    try {
      setLoading(true);
      const res = await api.get<MiniPassSeasonsResponse>("/admin/mini-pass/seasons");
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left text-sm min-w-[720px]">
              ...
              <tbody className="divide-y divide-slate-800">
                {rows.map((r) => (
                  <tr key={String(r.id)} className="hover:bg-slate-800/30">
