...
type AdminBackupsListResponse = { ok: true; backups?: AdminBackupRow[] } | { ok: false; message?: string };

type AdminBackupCreateResponse = {
  ok: true;
  backup?: Partial<AdminBackupRow> & {
    publicTableCount?: number;
    totalDataRows?: number;
    publicTablesWithRows?: number;
    bundleName?: string;
...
export default function AdminBackups() {
  ...
  const fetchBackups = useCallback(async () => {
    try {
      setIsLoading(true);
      const res = await api.get<AdminBackupsListResponse>("/admin/backups");
      if (res.data.ok) {
  ...
  const handleCreateBackup = async () => {
    ...
    try {
      ...
      const res = await api.post<AdminBackupCreateResponse>("/admin/backups");
      if (res.data.ok) {
  ...
  const handleDeleteBackup = async (filename: string) => {
    ...
    try {
      const res = await api.delete<{ ok: boolean }>("/admin/backups", { data: { filename } });
      if (res.data.ok) {
