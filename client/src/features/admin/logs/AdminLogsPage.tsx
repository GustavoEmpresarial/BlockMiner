...
function formatLogDetails(log: AdminAuditLogRow): string {
...
function auditLogTimestamp(log: AdminAuditLogRow): Date {
...
export default function AdminLogs() {
  ...
  const [matchedInWindow, setMatchedInWindow] = useState<number | null>(null);
  ...
  const fetchLogs = useCallback(
    async (nextOffset = 0, append = false) => {
      try {
        setIsLoading(true);
        const res = await api.get<AdminAuditListResponse>("/admin/audit", {
          params: {
            limit: PAGE_SIZE,
            offset: nextOffset,
  ...
  const filteredLogs = logs.filter((log: AdminAuditLogRow) => {
                ...
                <div className="flex flex-col gap-2">
                  <div className="flex items-start justify-between gap-3">
                    ...
                    <span className="shrink-0 text-right text-[10px] font-mono text-slate-500">
                      {auditLogTimestamp(log).toLocaleString()}
                    </span>
                  </div>
