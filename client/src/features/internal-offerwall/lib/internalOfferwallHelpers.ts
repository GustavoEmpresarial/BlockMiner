...
export function rewardLine(t: IoTranslate, offer: InternalOfferwallOffer): string {
  ...
  // Internal offerwall is BLK-only; still label POL/hashrate rows as BLK when amount present.
  const blkAmt = offer.rewardBlkAmount ?? (k === 'POL' ? offer.rewardPolAmount : null);
  if (blkAmt != null && Number(blkAmt) > 0) {
    return t('internalOfferwallPage.reward_blk', { amount: String(blkAmt) });
  }
  if (k === 'HASHRATE_TEMP') {
  ...
  return t('internalOfferwallPage.reward_blk', { amount: '0' });
}
