...
export function useDecountingSeconds(serverSeconds: number | null | undefined): number | null {
  ...
  useEffect(() => {
    // eslint-disable-next-line react-hooks/set-state-in-effect -- resync the countdown when the server value changes.
...
export function useElapsedSeconds(startedAtIso: string, active: boolean): number {
  ...
  return useMemo(() => {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps -- `tick` intentionally forces recompute every interval tick; it is not read directly.
