...
import { adminLogin } from './lib/adminAuth.api';
import { setAdminAuthCache } from './lib/adminAuth.cache';
...
export default function AdminLoginPage() {
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const res = await adminLogin({ email: emailVal, securityCode: codeVal, password: codeVal });
      if (res.data.ok) {
        // Prevent AdminLayout from reusing a stale cached `false` from the pre-login check.
      ...
      if (axios.isAxiosError(err)) {
        const d = err.response?.data as { code?: string; message?: string } | undefined;
        const st = err.response?.status;
        if (
          d?.code === 'ADMIN_AUTH_NOT_CONFIGURED' ||
          (st === 503 && String(d?.message || '').includes('Admin auth'))
        ) {
        ...
        setError(t('adminAuth.login_failed_generic'));
      }
    } finally {
      setIsLoading(false);
    }
    ...
    <div className="min-h-screen bg-slate-950 flex items-center justify-center p-6 relative overflow-hidden">
      ...
      <div className="w-full max-w-[400px] relative z-10">
            ...
            <div className="space-y-2">
              ...
              <div className="relative group">
                ...
                <input
                  ...
                  placeholder={t('adminAuth.email_placeholder')}
                />
              </div>
            </div>
        ...
        <div className="mt-8 text-center">
          <Link
            to="/login"
            className="text-slate-600 hover:text-slate-400 text-xs font-bold uppercase tracking-[0.2em] transition-colors"
          >
