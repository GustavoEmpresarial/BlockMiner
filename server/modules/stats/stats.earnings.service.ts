/**
 * Ported from legacy/server/modules/stats/stats.earnings.service.ts. Backs
 * GET /api/stats/earnings — consolidated POL earnings by source + cumulative daily history.
 */
import { Prisma } from "@prisma/client";
import prisma from "../../core/database/prisma.js";
import { REWARD_POL as INTERNAL_OFFERWALL_REWARD_POL } from "../internal-offerwall/index.js";
import { isAutoMiningV2SchemaAvailable } from "../auto-mining/index.js";
import { REFERRAL_STATS_SINCE } from "../referrals/index.js";
import { readStatsMaterializationMaxLagMs } from "../events/index.js";
import {
  parseEarningsPeriod,
...
export type { EarningsPeriod, EarningsTotals, EarningsHistoryPoint };
export {
  parseEarningsPeriod,
  rollupEarningsTotalsFromSourceMap,
  EARNINGS_STATS_PAYLOAD_KEYS,
...
function sinceFilter(
  since: Date | null,
  field: "createdAt" | "collectedAt" | "completedAt" | "callbackAt",
...
async function sumInternalOfferwallPol(userId: number, since: Date | null): Promise<number> {
  const attempts = await prisma.internalOfferwallAttempt.findMany({
    where: { userId, status: "COMPLETED", ...sinceFilter(since, "completedAt") },
...
async function sumInboxPolByCategory(
  ...
  const rows = await prisma.userRewardInbox.groupBy({
    by: ["source"],
    where: { userId, status: "collected", rewardType: "pol", ...sinceFilter(since, "collectedAt") },
...
async function fetchDailyInternalOfferwall(userId: number, since: Date | null): Promise<DailyRow[]> {
  const attempts = await prisma.internalOfferwallAttempt.findMany({
    where: { userId, status: "COMPLETED", ...sinceFilter(since, "completedAt") },
    select: { completedAt: true, offer: { select: { rewardKind: true, rewardPolAmount: true } } },
  });
...
function mergeDailyRows(rows: DailyRow[]): Map<string, EarningsHistoryPoint> {
  ...
  for (const row of byDate.values()) {
    ...
    row.total = roundPol(
      row.mining + row.offerwall + row.faucet + row.shortlinks + row.youtube + row.games + row.autoMining +
        row.checkin + row.referrals,
    );
  }
...
async function loadPowerMeta(userId: number, now: Date): Promise<UserEarningsPayload["powerMeta"]> {
  ...
  const [machineCount, gameBoosts, ytBoosts, gpuBoosts, v2Boosts, gainedGames, gainedYt, gainedShort, gainedGpu] =
    await Promise.all([
      ...
      v2Ok
        ...
        : Promise.resolve(0),
      prisma.userPowerGame.aggregate({ _sum: { hashRate: true }, where: { userId, playedAt: { gte: dayAgo } } }),
      prisma.youtubeWatchPower.aggregate({ _sum: { hashRate: true }, where: { userId, claimedAt: { gte: dayAgo } } }),
      prisma.shortlinkPower.aggregate({ _sum: { hashRate: true }, where: { userId, claimedAt: { gte: dayAgo } } }),
      prisma.autoMiningGpu.aggregate({ _sum: { gpuHashRate: true }, where: { userId, claimedAt: { gte: dayAgo } } }),
    ]);
  ...
  return { machineCount, activeBoosts, powerGained24h: Math.round(powerGained24h) };
}
...
async function tryBuildFromMaterialization(
  userId: number,
  period: EarningsPeriod,
  since: Date | null,
  now: Date,
): Promise<UserEarningsPayload | null> {
  try {
    const maxLagMs = readStatsMaterializationMaxLagMs();
    const newest = await prisma.userStatsEarningsTotal.findFirst({
      where: { userId },
      orderBy: { updatedAt: "desc" },
      select: { updatedAt: true },
    });
    if (!newest) return null;
    if (maxLagMs > 0 && now.getTime() - newest.updatedAt.getTime() > maxLagMs) {
      return null;
    }

    const totalRows = await prisma.userStatsEarningsTotal.findMany({
      where: { userId },
      select: { source: true, amountPol: true },
    });
    if (totalRows.length === 0) return null;

    const dailyRows = await prisma.userStatsEarningsDaily.findMany({
      where: {
        userId,
        ...(since ? { dayUtc: { gte: since } } : {}),
      },
      select: { dayUtc: true, source: true, amountPol: true },
      orderBy: { dayUtc: "asc" },
    });
    ...
    if (since == null) {
      for (const row of totalRows) {
        bySource[row.source] = roundPol(Number(row.amountPol));
      }
    } else {
      // Period-scoped totals must match the selected window (not all-time lifetime counters).
      for (const row of dailyRows) {
        const key = row.source;
        bySource[key] = roundPol((bySource[key] ?? 0) + Number(row.amountPol));
      }
    }
    const totals = rollupEarningsTotalsFromSourceMap(bySource);

    const mapped: DailyRow[] = dailyRows.map((r) => ({
      day: r.dayUtc,
      pol: Number(r.amountPol),
      category: r.source,
    }));
    const history = toCumulativeHistory(Array.from(mergeDailyRows(mapped).values()));
    const powerMeta = await loadPowerMeta(userId, now);
    ...
    // Tables missing / prisma client not generated yet — fall back to live.
    return null;
  }
}
