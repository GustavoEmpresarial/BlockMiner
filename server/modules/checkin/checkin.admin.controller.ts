...
export async function listCheckinMilestones(_req: Request, res: Response): Promise<void> {
  try {
    const rows = await prisma.checkinStreakMilestone.findMany({
      ...
      include: { miner: { select: { id: true, name: true, baseHashRate: true, isActive: true, isArchived: true } } },
    });
    res.json({
      ok: true,
      milestones: rows.map((m) => ({ ...m, rewardValue: Number(m.rewardValue), minerName: m.miner?.name ?? null })),
    });
  } catch (e: unknown) {
    log.error("admin listCheckinMilestones", { error: String(e) });
    if (isMissingMilestoneTablesError(e)) {
      res.status(503).json({ ok: false, code: "MILESTONE_DB_PENDING", message: "Check-in milestone tables are missing. Apply pending Prisma migrations on the server, then retry." });
      return;
    }
...
export async function createCheckinMilestone(req: Request<unknown, unknown, MilestoneBody>, res: Response): Promise<void> {
  try {
    ...
    log.error("admin createCheckinMilestone", { error: String(e) });
...
export async function updateCheckinMilestone(req: Request<IdParams, unknown, MilestoneBody>, res: Response): Promise<void> {
  try {
    ...
    log.error("admin updateCheckinMilestone", { error: String(e) });
...
export async function deleteCheckinMilestone(req: Request<IdParams>, res: Response): Promise<void> {
  try {
    ...
    log.error("admin deleteCheckinMilestone", { error: String(e) });
