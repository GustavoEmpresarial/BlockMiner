...
import { MINING_MODES, DAILY_LIMIT_HASH, CYCLE_SECONDS, isClaimDue, canGrantDaily, validateImpressionForTurboClaim, assertValidMiningMode, nextClaimAfterSuccess, hashRateForMode, CLAIM_REQUIRED_SECONDS, HEARTBEAT_STALE_MS, CLAIM_SECONDS_COST, CLICK_GRACE_MS, MIN_CLICK_DELAY_MS, isHeartbeatStale, utcDayDailyResetMeta, endOfUtcCalendarDay, startOfUtcCalendarDay, } from "./auto-mining.domain.js";
...
async function hasActiveBoost(userId, now = new Date()) {
...
async function hasActiveBoostTx(tx, userId, now = new Date()) {
...
async function resolveRewardExpiresAtForGrant(tx, userId, earnedAt) {
...
async function getUserPresence(userId, tx = prisma) {
...
function notReadyError(code, extra) {
    const err = new Error(code);
    err.code = code;
    if (extra.retryAfterMs != null)
        err.retryAfterMs = extra.retryAfterMs;
    if (extra.secondsShort != null)
        err.secondsShort = extra.secondsShort;
    return err;
}
/** While the tab was away, do not auto-grant missed cycles — restart the countdown on return. */
async function resyncSessionAfterAbsence(userId, session, now, tx = prisma) {
    ...
    if (await hasActiveBoost(userId, now))
        return session;
...
export async function resumeSession(userId) {
    ...
    if (!session)
        return { resumed: false };
    const remainingMs = Math.min(CYCLE_SECONDS * 1000, Math.max(0, session.pausedRemainingMs ?? CYCLE_SECONDS * 1000));
    const res = await prisma.autoMiningV2Session.updateMany({
...
export async function deactivateStaleAutoMiningSessions(staleMs = 30 * 60 * 1000) {
    if (!(await isAutoMiningV2SchemaAvailable()))
        return 0;
