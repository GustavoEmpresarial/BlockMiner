...
export async function getActiveBroadcast(req, res) {
...
export async function dismissBroadcast(req, res) {
    const user = requireSessionUser(req, res);
    if (!user)
        return;
    try {
...
function normalizeBroadcastLabel(v) {
    if (v === undefined)
        return undefined;
    if (v === null)
        return null;
...
export function uploadBroadcastImage(req, res) {
...
export async function adminListBroadcasts(_req, res) {
    try {
...
export async function adminPatchBroadcast(req, res) {
    try {
        const id = Number(req.params.id);
        const { title, content, imageUrl, isActive, dismissDelaySeconds, linkUrl, linkLabel, linkNewTab } = req.body;
        if (isActive)
            await broadcastService.deactivateOtherBroadcastMessages(id);
        const delay = clampDismissDelay(dismissDelaySeconds);
        const link = normalizeBroadcastLink(linkUrl);
        const label = normalizeBroadcastLabel(linkLabel);
