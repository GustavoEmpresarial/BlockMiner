...
import * as rewardInboxService from "./reward-inbox.service.js";

export async function getRewardInbox(req: Request, res: Response): Promise<void> {
  try {
    const user = requireSessionUser(req, res);
    if (!user) return;
    await rewardInboxService.applyPendingTournamentPowerInbox(user.id);
...
export async function collectReward(req: Request, res: Response): Promise<void> {
  try {
    const user = requireSessionUser(req, res);
    if (!user) return;
    ...
    const result = await rewardInboxService.collectInboxItem(user.id, inboxId);
    res.json({ ok: true, rewardType: result.rewardType });
  } catch (e: unknown) {
...
export async function collectAllRewards(req: Request, res: Response): Promise<void> {
  try {
    const user = requireSessionUser(req, res);
    if (!user) return;
