...
import { isPendingTournamentPowerInboxItem, isTemporaryPowerRewardType, resolvePowerBoostGame, } from "./reward-inbox.power.js";
import * as repo from "./reward-inbox.repository.js";
function mapInboxSourceToEarningsSource(source) {
    const s = source.toLowerCase();
    if (s === "checkin_milestone")
        return "checkin";
    if (s === "faucet" || s === "faucet_power")
        return "faucet";
    if (s === "shortlink" || s === "shortlinks")
        return "shortlinks";
    if (s === "youtube" || s === "youtuber_reward")
        return "youtube";
    if (s === "auto_mining" || s === "auto_mining_v2")
        return "autoMining";
    if (s === "tournament" || s === "game" || s === "games" || s === "daily_task" || s === "burn_event") {
        return "games";
    }
    if (s === "offerwall" || s === "internal_offerwall")
        return "offerwallInternal";
    return null;
}
function readInboxEventMinerId(metaJson) {
    if (!metaJson || typeof metaJson !== "object" || Array.isArray(metaJson))
        return null;
    const raw = metaJson.eventMinerId;
    const id = Number(raw);
    return Number.isInteger(id) && id > 0 ? id : null;
}
export async function grantTemporaryPowerInTx(tx, userId, item) {
...
export async function applyPendingTournamentPowerInbox(userId) {
...
export async function collectInboxItem(userId, inboxId) {
    ...
    await prisma.$transaction(async (tx) => {
        ...
        if (rt === "pol") {
            ...
            if (!item.minerName)
                throw new Error("MISSING_MINER_NAME");
    ...
    return { ok: true, rewardType: item.rewardType };
}
export async function collectAllPendingForUser(userId) {
    const items = await repo.listPendingInboxForUser(userId);
    const results = await Promise.allSettled(items.map((item) => collectInboxItem(userId, item.id)));
    const succeeded = results.filter((r) => r.status === "fulfilled").length;
    const failed = results.filter((r) => r.status === "rejected").length;
    const machinesCollected = results.filter((r) => r.status === "fulfilled" && r.value.rewardType === "machine").length;
    return { ok: true, collected: succeeded, failed, machinesCollected };
}
