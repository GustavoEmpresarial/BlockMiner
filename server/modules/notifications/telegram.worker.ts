...
import type { TelegramOutboxEvent } from "@prisma/client";
...
export function isTelegramOutboxWorkerRunning(): boolean {
  return workerRunning;
}
export function setTelegramOutboxWorkerRunning(value: boolean): void {
  workerRunning = value;
}
...
export function getWorkerConfig(): TelegramWorkerConfig {
...
function buildPolygonscanTxUrl(txHash: string | null, baseUrl = normalizePolygonscanBaseUrl()): string {
...
function payloadOf(event: TelegramOutboxEvent): OutboxPayload {
  return (event.payload as OutboxPayload) || {};
}

export function buildPrivateAlertMessage(event: TelegramOutboxEvent): string {
...
export function buildAutoSentAlertMessage(event: TelegramOutboxEvent): string {
  const payload = payloadOf(event);
  const username = esc(payload.username || event.usernameSnapshot || `user-${event.userId || "?"}`);
...
export function buildHotWalletLowBalanceMessage(event: TelegramOutboxEvent): string {
  const payload = payloadOf(event);
...
export function buildPublicProofMessage(event: TelegramOutboxEvent, baseUrl = normalizePolygonscanBaseUrl()): string {
  const payload = payloadOf(event);
  const txHash = cleanString(event.txHash || payload.txHash);
...
export async function sendTelegramMessage(params: {
  ...
  if (!chatId) throw new Error("chat_id nao configurado.");
...
async function claimNextEvent(): Promise<TelegramOutboxEvent | null> {
...
async function markSent(eventId: number): Promise<void> {
...
async function markFailed(event: TelegramOutboxEvent, error: unknown, config: TelegramWorkerConfig): Promise<void> {
...
const GENERIC_EVENT_TYPES: readonly string[] = [
...
export async function processTelegramEvent(
  ...
  config: TelegramWorkerConfig = getWorkerConfig(),
  fetchImpl?: TelegramFetchImpl,
): Promise<{ sent: true }> {
 ...
 */
export async function runTelegramOutboxTick(
  overrides: { config?: TelegramWorkerConfig; fetchImpl?: TelegramFetchImpl } = {},
