/**
 * Port of legacy/server/modules/antibot/domain/weights.ts (configurable evidence weights +
 * tiered corroboration scoring). Every weight can be overridden through an env var named
 * `ANTIBOT_W_<CODE>` (uppercased). The engine never blocks on a single signal — these values
 * are *additive*.
 */
import type { EvidenceSeverity } from "./antibot.types.js";
 ...
 */
export function resolveWeight(code: string): WeightEntry {
 ...
 */
export const STRONG_CODES = new Set<string>([
  "navigator_webdriver",
  "headless_browser",
  "selenium",
  "puppeteer",
...
export type CodeTier = "strong" | "structural" | "weak";

export function tierForCode(code: string): CodeTier {
 ...
 */
export function computeScoreFromActiveCodes(
  active: Array<{ code: string; weight: number }>,
): { score: number; corroborated: boolean } {
