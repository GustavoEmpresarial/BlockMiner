...
export function safeBackupSqlName(name) {
...
export function safeBackupBundleName(name) {
    ...
    if (name.includes("..") || name.includes("/") || name.includes("\\"))
        return null;
    if (!/^backup-.+\.bundle\.tar\.gz$/i.test(name))
        return null;
    return name;
}
/**
    ...
    return new Promise((resolve, reject) => {
        ...
        child.on("close", (code) => {
            if (code === 0)
                ...
                reject(new Error(`tar exited with code ${code}: ${stderr.slice(-8000) || "no stderr"}`));
        });
    });
}
        ...
        try {
            ...
            if (meta && typeof meta === "object") {
                if (meta.status === "success" || meta.status === "failed")
                    status = meta.status;
                if (Number.isFinite(meta.publicTableCount))
                    publicTableCount = meta.publicTableCount;
                if (Number.isFinite(meta.durationMs))
                    durationMs = meta.durationMs;
                if (Number.isFinite(meta.copyPublicLineCount))
                    copyPublicLineCount = meta.copyPublicLineCount;
                if (Array.isArray(meta.criticalTablesPresent))
                    criticalTablesPresent = meta.criticalTablesPresent;
                const audit = meta.rowCountAudit;
                if (meta.bundle && typeof meta.bundle === "object") {
                    if (typeof meta.bundle.name === "string")
                        bundleName = meta.bundle.name;
                    if (Number.isFinite(meta.bundle.size))
                        bundleSize = meta.bundle.size;
                    if (Array.isArray(meta.bundle.includedPaths))
                        bundleIncludedPaths = meta.bundle.includedPaths;
                }
                if (audit && typeof audit === "object" && audit.mode === "exact_count") {
                    if (Number.isFinite(audit.totalDataRows))
                        totalDataRows = audit.totalDataRows;
                    if (Number.isFinite(audit.publicTablesWithRows))
                        publicTablesWithRows = audit.publicTablesWithRows;
                    if (Number.isFinite(audit.publicTablesEmpty))
                        publicTablesEmpty = audit.publicTablesEmpty;
...
export async function resolveBackupBundleDownloadPath(filename) {
    ...
    if (!safe)
        throw new Error("Invalid backup bundle filename");
    const backupsDir = path.resolve(getAdminBackupsDirectory());
    const full = path.resolve(path.join(backupsDir, safe));
    const rel = path.relative(backupsDir, full);
    if (rel.startsWith("..") || path.isAbsolute(rel))
