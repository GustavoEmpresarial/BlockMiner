...
} from "./sidebar-nav.registry.js";
import type { SidebarCategory, SidebarPersistedEntry, SidebarSection } from "./sidebar-nav.types.js";

const SINGLETON_ID = 1;

export function buildResolvedCategories(entries: SidebarPersistedEntry[]): SidebarCategory[] {
  ...
  function isShown(entry: SidebarPersistedEntry): boolean {
...
export async function getSidebarNavForAdmin(): Promise<{
...
export async function getSidebarNavCategoriesPublic(): Promise<SidebarCategory[]> {
  const { categories } = await getSidebarNavForAdmin();
  return categories;
}

export async function saveSidebarNavEntries(
  bodyEntries: unknown,
): Promise<
  | { ok: true; entries: SidebarPersistedEntry[]; categories: SidebarCategory[] }
...
export function collectVisiblePathsFromCategories(categories: unknown): Set<string> {
...
export async function getVisibleSidebarPaths(): Promise<Set<string>> {
  const categories = await getSidebarNavCategoriesPublic();
  return collectVisiblePathsFromCategories(categories);
}

export async function isSidebarPathVisible(path: string): Promise<boolean> {
