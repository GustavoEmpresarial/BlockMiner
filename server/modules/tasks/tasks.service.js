...
import { STATUS_AVAILABLE, STATUS_CLAIMED, STATUS_COMPLETED, STATUS_IN_PROGRESS, TASK_INTERNAL_OFFERWALL, TASK_LOGIN_DAY, } from "./tasks.constants.js";
import { getDailyTaskPeriodKey, getNextDailyTaskResetAt, normalizeDailyTaskResetCadence, } from "./tasks.period.js";
 ...
 */
export async function bumpDailyTasksForUser(userId, taskType, opts) {
    ...
    for (const def of defs) {
        if (taskType === TASK_INTERNAL_OFFERWALL) {
            ...
            if (scoped != null) {
                if (internalOfferwallOfferId == null || scoped !== internalOfferwallOfferId)
                    continue;
            }
        }
        if (def.gameSlug) {
            if (!gameSlug || def.gameSlug !== gameSlug)
                continue;
        }
        ...
        await prisma.$transaction(async (tx) => {
            ...
            if (!row || row.rewardClaimedAt || row.completedAt)
                return;
...
export async function notifyDailyTaskGamePlayed(userId, args) {
    if (!userId || !args.userPowerGameId)
        return;
...
export async function notifyDailyTaskInternalOfferwallCompleted(userId, attemptId, internalOfferwallOfferId) {
    if (!userId || !attemptId)
        return;
...
function rewardSummary(def) {
    ...
    if (kind === "HASHRATE_TEMP")
        return { kind, hashRate: def.rewardHashRate ?? 0, days: def.rewardHashRateDays ?? 1 };
...
export async function filterDailyTaskDefsForSidebar(defs) {
    ...
    return defs.filter((def) => {
        if (def.taskType === TASK_LOGIN_DAY)
            return checkinOn;
        if (def.taskType === TASK_INTERNAL_OFFERWALL)
            return Boolean(offerwallOn);
        return true;
    });
}
export async function getDailyTasksDashboard(userId) {
    ...
    for (const def of defs) {
        ...
        if (Number.isFinite(resetAt) && resetAt < nextResetAtMs)
...
export async function claimDailyTaskReward(userId, taskDefinitionId) {
    try {
        ...
        const out = await prisma.$transaction(async (tx) => {
            ...
            if (inboxPayload)
                await createRewardInboxEntry(tx, inboxPayload);
        ...
        if (code === "NOT_COMPLETED")
            return { ok: false, code: "not_completed", status: 400 };
