/**
 * Ported from legacy/server/services/dailyTasks/dailyTaskDefinitionAdminValidation.ts.
 */
import { Prisma } from "@prisma/client";
import { ADMIN_CREATE_REWARD_KINDS, ADMIN_CREATE_TASK_TYPES, TASK_INTERNAL_OFFERWALL } from "./tasks.constants.js";
import { DAILY_TASK_RESET_CADENCES, normalizeDailyTaskResetCadence } from "./tasks.period.js";
...
export type ParsePatchDailyTaskDefinitionResult =
  | {
      ok: true;
      data: Prisma.DailyTaskDefinitionUncheckedUpdateInput;
      needsMinerId: number | null;
      needsEventMinerId: number | null;
      needsOfferwallId: number | null;
    }
  | { ok: false; status: number; message: string };
...
export function parsePatchDailyTaskDefinition(body: unknown): ParsePatchDailyTaskDefinitionResult {
  ...
  if (b.taskType !== undefined) {
    ...
    if (!ADMIN_CREATE_TASK_TYPES.includes(tt)) return { ok: false, status: 400, message: "Invalid task type." };
    data.taskType = tt;
  }
  ...
  if (b.validFrom !== undefined) {
    if (b.validFrom === null || String(b.validFrom).trim() === "") {
      ...
      if (isNaN(d.getTime())) return { ok: false, status: 400, message: "Invalid validFrom date." };
      data.validFrom = d;
    }
  }
  ...
  if (b.validUntil !== undefined) {
    if (b.validUntil === null || String(b.validUntil).trim() === "") {
      ...
      if (isNaN(d.getTime())) return { ok: false, status: 400, message: "Invalid validUntil date." };
      data.validUntil = d;
    }
  }
  ...
  if (b.sortOrder !== undefined && b.sortOrder !== null) {
    ...
    if (!Number.isInteger(n) || n < 0 || n > 99999) return { ok: false, status: 400, message: "Invalid sort order." };
    data.sortOrder = n;
  }
  ...
  if (b.targetValue !== undefined) {
    ...
    if (!Number.isFinite(n) || n <= 0 || n > 1e15) return { ok: false, status: 400, message: "Invalid target value (must be positive)." };
  ...
  if (b.translationKey !== undefined) {
    ...
    if (!key || key.length > 120 || !I18N_KEY_RE.test(key)) return { ok: false, status: 400, message: "Invalid translation key." };
  ...
  if (b.gameSlug !== undefined) {
    if (b.gameSlug === null || String(b.gameSlug).trim() === "") {
      ...
      if (g.length > 64 || !/^[a-z0-9_-]+$/i.test(g)) return { ok: false, status: 400, message: "Invalid game slug." };
      data.gameSlug = g;
    }
  }
  ...
  if (b.internalOfferwallOfferId !== undefined) {
    if (b.internalOfferwallOfferId === null || String(b.internalOfferwallOfferId).trim() === "") {
      ...
      if (!Number.isInteger(oid) || oid < 1) return { ok: false, status: 400, message: "Invalid internalOfferwallOfferId." };
  ...
  if (b.rewardKind !== undefined) {
    ...
    if (!ADMIN_CREATE_REWARD_KINDS.includes(rewardKind)) return { ok: false, status: 400, message: "Invalid reward kind." };
    ...
    if (rewardKind === "BLK") {
      ...
      if (!Number.isFinite(a) || a <= 0 || a > 1e9) return { ok: false, status: 400, message: "BLK reward requires a positive rewardBlkAmount." };
      data.rewardBlkAmount = new Prisma.Decimal(String(a));
    } else if (rewardKind === "POL") {
      const a = parseFloat(String(b.rewardPolAmount ?? ""));
      if (!Number.isFinite(a) || a <= 0 || a > 1e9) return { ok: false, status: 400, message: "POL reward requires a positive rewardPolAmount." };
      ...
      if (!Number.isFinite(hr) || hr <= 0 || hr > 1e9) return { ok: false, status: 400, message: "HASHRATE_TEMP requires a positive rewardHashRate." };
      ...
      if (!Number.isInteger(mid) || mid < 1) return { ok: false, status: 400, message: "SHOP_MINER requires a valid rewardMinerId." };
      needsMinerId = mid;
      data.rewardMinerId = mid;
    } else if (rewardKind === "EVENT_MINER") {
      const eid = parseInt(String(b.rewardEventMinerId ?? ""), 10);
      if (!Number.isInteger(eid) || eid < 1) return { ok: false, status: 400, message: "EVENT_MINER requires a valid rewardEventMinerId." };
      needsEventMinerId = eid;
      data.rewardEventMinerId = eid;
    }
  }

  if (Object.keys(data).length === 0) return { ok: false, status: 400, message: "No valid fields to update." };
