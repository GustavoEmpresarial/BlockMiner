...
export type AbstractapiResponse = {
  ip_address?: unknown;
  security?: {
    is_vpn?: unknown;
    is_proxy?: unknown;
...
export function parseAbstractapiResponse(payload: unknown): ProxyLookupResult {
  if (!payload || typeof payload !== "object" || Array.isArray(payload)) {
    return { source: ABSTRACTAPI_SOURCE, error: "provider_error" };
  }
  const body = payload as AbstractapiResponse;
...
export async function lookupAbstractapi(
  ip: string,
  ...
  try {
    ...
    if (!res.ok) return { source: ABSTRACTAPI_SOURCE, error: "provider_error" };
