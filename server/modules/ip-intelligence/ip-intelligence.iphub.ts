...
export function parseIphubResponse(payload: unknown): ProxyLookupResult {
  if (!payload || typeof payload !== "object" || Array.isArray(payload)) {
    return { source: IPHUB_SOURCE, error: "provider_error" };
  }
  const body = payload as IphubResponse;
...
export async function lookupIphub(ip: string, { fetchImpl = globalThis.fetch }: ProxyLookupDeps = {}): Promise<ProxyLookupResult> {
  ...
  try {
    ...
    if (!res.ok) return { source: IPHUB_SOURCE, error: "provider_error" };
