...
export function parseVpnapiResponse(payload: unknown): ProxyLookupResult {
  if (!payload || typeof payload !== "object" || Array.isArray(payload)) {
    return { source: VPNAPI_SOURCE, error: "provider_error" };
  }
  const body = payload as VpnapiResponse;
...
export async function lookupVpnapi(ip: string, { fetchImpl = globalThis.fetch }: ProxyLookupDeps = {}): Promise<ProxyLookupResult> {
  ...
  try {
    ...
    if (!res.ok) return { source: VPNAPI_SOURCE, error: "provider_error" };
