...
const TRUSTED_HEADER_NAMES: Record<string, string> = {
...
function configuredHeaderPriority(): string[] {
...
function readHeaderIp(req: RequestLike, headerName: string): string | null {
...
export function resolveClientIp(req: RequestLike) {
...
export function getClientIpFromResolver(req: RequestLike): string {
...
export function deriveDefaultNetworkCidr(ip: unknown): string | null {
  const normalized = normalizeIp(ip);
