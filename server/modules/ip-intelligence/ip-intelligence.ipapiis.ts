...
export function parseIpapiisResponse(payload: unknown): ProxyLookupResult {
  if (!payload || typeof payload !== "object" || Array.isArray(payload)) {
    return { source: IPAPIIS_SOURCE, error: "provider_error" };
  }
  const body = payload as IpapiisResponse;
...
export async function lookupIpapiis(ip: string, { fetchImpl = globalThis.fetch }: ProxyLookupDeps = {}): Promise<ProxyLookupResult> {
  ...
  try {
    ...
    if (!res.ok) return { source: IPAPIIS_SOURCE, error: "provider_error" };
