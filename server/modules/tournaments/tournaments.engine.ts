...
import {
  isOfferwallIncrementalMetric,
  isMinigameIncrementalMetric,
} from "./tournaments.metrics.js";
import {
...
const INCREMENTAL_METRICS: TournamentMetric[] = [
  ...
  ...(ACTION_INCREMENTAL_METRICS as unknown as TournamentMetric[]),
];
...
export type MiningBlockContribution = {
  tournamentId: number;
  userId: number;
...
export function buildMiningBlockContributions(
  tournaments: ReadonlyArray<{ id: number; startsAt: Date; endsAt: Date }>,
  payload: MiningBlockSettledPayload,
): MiningBlockContribution[] {
  const eventAt = new Date(payload.eventAt);
  if (!Number.isFinite(eventAt.getTime())) throw new Error("Invalid mining block event timestamp");
  return tournaments
    ...
    .flatMap((tournament) =>
      payload.userIds.map((userId) => ({
        tournamentId: tournament.id,
        userId,
        sourceType: "BLOCKS_MINED" as const,
...
export async function handleMiningBlockSettled(payload: MiningBlockSettledPayload): Promise<void> {
  ...
  await Promise.all(
    Array.from({ length: Math.min(concurrency, tasks.length) }, () => processNext()),
  );
}
...
export async function reconcileTournament(
  tournamentId: number,
  options: { recomputeBlocks?: boolean } = {},
...
export async function reconcileLegacyBatchTournament(tournamentId: number): Promise<void> {
  ...
  if (
    (INCREMENTAL_METRICS as readonly string[]).includes(tournament.metric) ||
