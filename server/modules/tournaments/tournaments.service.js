...
import { backfillMinigameTournamentFromLogs, resolveTournamentStatusForWindow, } from "./tournaments.minigame-backfill.js";
import { isTournamentSkipGetRecomputeEnabled, isTournamentIncrementalScoringEnabled, } from "./tournaments.flags.js";
...
import { getCachedLeaderboard, setCachedLeaderboard, invalidateLeaderboardCache, } from "./tournaments.cache.js";
...
export async function alignActiveTournamentWindows() {
    const now = new Date();
    ...
    for (const t of active) {
        ...
        const isOfferwall = OFFERS_INCREMENTAL_METRICS.includes(t.metric);
        const isMinigame = MINIGAME_INCREMENTAL_METRICS.includes(t.metric);
        ...
        fixed++;
        log.info(`[tournaments] aligned #${t.id} "${t.name}" → UTC ${snap.start.toISOString()} .. ${snap.end.toISOString()}`);
    }
    return fixed;
}
function nextCycleWindow(type, prevEnd, duration) {
    const snap = snapWindowForType(type, prevEnd);
    if (!snap)
        return { newStart: prevEnd, newEnd: new Date(prevEnd.getTime() + duration) };
    let { start, end } = snap;
    if (start < prevEnd) {
...
export async function finalizeTournament(tournamentId) {
    ...
    if (tournament.status === "ENDED")
        return { ranked: 0, rewarded: 0, nextId: null };
    ...
    let rewarded = 0;
    const powerGrantUserIds = new Set();
        ...
        for (let i = 0; i < entries.length; i++) {
            ...
            if (prize && grantedUpdate.count === 1) {
                ...
                if (prize.prizeType === "MINING_BOOST")
                    powerGrantUserIds.add(entry.userId);
                rewarded++;
            }
...
async function invalidateTournamentCaches(tournamentId, metric) {
    await invalidateLeaderboardCache(tournamentId);
    invalidateActiveTournamentsCache();
    invalidateTournamentCache(metric);
}
async function countInboxGrantsForEntryTx(tx, userId, entryId) {
...
async function grantPrize(tx, ctx) {
...
export async function getTypeDisplayOrder() {
...
export async function setTypeDisplayOrder(typeOrder) {
...
function sortByTypeOrder(list, order) {
    const idx = (t) => {
...
export function invalidateActiveTournamentsCache() {
...
const PLAYER_HUB_HIDDEN_METRICS = ["FAUCET", "SHORTLINK", "AUTO_MINING"];
...
async function enrichDepositLeaderboardWithPolTotals(top, startsAt, upperBound) {
    ...
    const polByUser = new Map(rows.map((r) => [r.userId, Number(r._sum.amount ?? 0)]));
    return top.map((entry) => ({
        ...entry,
        scorePol: polByUser.get(entry.userId) ?? 0,
    }));
}
export async function getTournamentWithLeaderboard(tournamentId, userId) {
    ...
    let top = await getCachedLeaderboard(tournamentId);
    if (!top) {
    ...
    if (userId) {
        ...
        if (myEntry) {
            ...
            if (rank != null) {
                const prize = tournament.prizes.find((p) => rank >= p.rankFrom && rank <= p.rankTo);
                if (prize)
                    myPrize = { prizeType: prize.prizeType };
            }
        }
        if (depositMetric) {
            registerTournamentMetricScorers();
            ...
            if (scorer?.getUserBreakdown) {
                myDepositBreakdown = (await scorer.getUserBreakdown(userId, {
                    id: tournament.id,
                    name: tournament.name,
                    metric: tournament.metric,
                ...
                myDepositBreakdown = await getDepositScoreDetailForUser(userId, tournament.startsAt, upperBound);
            }
        }
    }
    return {
        tournament: {
            ...
            windowUtcNow: formatUtcWindowLabel(tournament.startsAt, tournament.endsAt < new Date() ? tournament.endsAt : new Date()),
        },
        top,
        myEntry,
        myRankLive,
        ...
        myDepositBreakdown: depositMetric && myDepositBreakdown
            ? { breakdown: myDepositBreakdown.breakdown }
            : null,
    };
}
export async function getUserTournamentHistory(userId) {
...
export async function adminUpdateTournament(tournamentId, data) {
...
export async function adminCancelTournament(tournamentId) {
    ...
    if (!t)
        throw new Error("Tournament not found");
    if (t.status === "ENDED")
        throw new Error("Cannot cancel an ended tournament");
...
export async function adminGetEntries(tournamentId, page = 1, limit = 50) {
...
export { tournamentUpperBound, isAuditableMetric, adminTournamentScoreAudit, adminTournamentScoreAuditUser, getMyTournamentScoreBreakdown, } from "./tournaments.audit.js";
