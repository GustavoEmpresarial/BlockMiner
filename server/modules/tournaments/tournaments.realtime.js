...
let subscriber = null;
function publishDirty(tournamentId) {
    const redis = getRedis();
    if (!redis)
        return;
    redis.publish(DIRTY_CHANNEL, String(tournamentId)).catch((err) => {
...
export function notifyTournamentDirty(tournamentId) {
...
export function markTournamentDirty(tournamentId) {
    if (!ioRef)
        return;
...
export async function buildLeaderboardSlice(tournamentId) {
...
export async function emitTournamentUpdateNow(tournamentId) {
    if (!ioRef)
        return;
    const t0 = Date.now();
    try {
