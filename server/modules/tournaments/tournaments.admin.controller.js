import { adminListTournaments, adminCreateTournament, adminUpdateTournament, adminCancelTournament, adminGetEntries, finalizeTournament, adminTournamentScoreAudit, adminTournamentScoreAuditUser, getTypeDisplayOrder, setTypeDisplayOrder, } from "./tournaments.service.js";
import { getEngineStats } from "./tournaments.metrics.js";
import { listRecentDriftAlerts, listShadowValidationAlerts, } from "./tournaments.offerwall-drift.js";
...
function validatePrizeAmount(value, label) {
    if (value == null)
        return null;
...
function validatePrizes(prizes) {
    ...
    for (const p of prizes) {
        if (typeof p !== "object" || p === null)
            return "Invalid prize entry";
        const r = p;
        if (typeof r.rankFrom !== "number" || typeof r.rankTo !== "number")
            return "Invalid prize rank range";
        if (r.rankFrom < 1 || r.rankTo < r.rankFrom)
            return "Invalid prize rank range";
        const err = validatePrizeAmount(r.polAmount, "polAmount") ??
...
export async function create(req, res) {
    const { name, description, type, metric, startsAt, endsAt, prizes, recurring } = req.body;
...
export async function update(req, res) {
    ...
    if (Array.isArray(body.prizes)) {
        ...
        patch.prizes = body.prizes;
    }
    const finalStart = patch.startsAt ?? (body.startsAt ? new Date(body.startsAt) : null);
    const finalEnd = patch.endsAt ?? (body.endsAt ? new Date(body.endsAt) : null);
...
export async function getDisplayOrder(_req, res) {
    try {
...
export async function updateDisplayOrder(req, res) {
    const body = req.body;
    if (!Array.isArray(body.typeOrder) ||
...
export async function scoreAudit(req, res) {
    const id = parseInt(String(req.params.id ?? ""), 10);
    if (!id) {
        res.status(400).json({ ok: false, message: "Invalid id" });
        return;
...
export async function scoreAuditUser(req, res) {
    const id = parseInt(String(req.params.id ?? ""), 10);
...
export async function engineStats(req, res) {
    const id = parseInt(String(req.params.id ?? ""), 10);
    if (!id) {
        res.status(400).json({ ok: false, message: "Invalid id" });
        return;
...
export async function driftAlerts(req, res) {
    const id = parseInt(String(req.params.id ?? ""), 10);
    if (!id) {
        res.status(400).json({ ok: false, message: "Invalid id" });
        return;
...
export async function offerwallMigration(req, res) {
    const id = parseInt(String(req.params.id ?? ""), 10);
    if (!id) {
        res.status(400).json({ ok: false, message: "Invalid id" });
        return;
...
export async function shadowAlerts(req, res) {
    const id = parseInt(String(req.params.id ?? ""), 10);
    if (!id) {
        res.status(400).json({ ok: false, message: "Invalid id" });
        return;
