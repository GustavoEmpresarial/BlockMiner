...
export function aggregateUserHashrates(
  user: HashrateUser,
  opts: { onlyActiveMiners?: boolean } = {},
) {
...
export function rankingUserSelect(now: Date, opts: { includeAutoMiningV2?: boolean } = {}) {
...
export function buildRankingRows(users: HashrateUser[]) {
...
export async function loadUsersForHashrateTournament(
  now: Date,
  v2SchemaOk: boolean,
): Promise<HashrateUser[]> {
  ...
  }) as Promise<HashrateUser[]>;
}

export type RankingRow = ReturnType<typeof buildRankingRows>[number];
...
let rankingCache: { rows: RankingRow[]; computedAt: number } | null = null;
let rankingRefreshInFlight: Promise<RankingRow[]> | null = null;

async function computeRankingRows(now: Date, v2SchemaOk: boolean): Promise<RankingRow[]> {
 ...
 */
export async function getCachedRankingRows(now: Date, v2SchemaOk: boolean): Promise<RankingRow[]> {
  const fresh = rankingCache && Date.now() - rankingCache.computedAt < RANKING_CACHE_TTL_MS;
  if (rankingCache && fresh) return rankingCache.rows;
