...
import {
  ...
  type MiningBlockSettledPayload,
} from "./tournaments.types.js";
import {
  handleDepositConfirmed,
  handleMiningBlockSettled,
  handleTournamentAction,
} from "./tournaments.engine.js";
...
export type TournamentOutboxDispatch =
  | "deposit_confirmed"
  | "action_recorded"
  | "block_mined"
  | null;

export function resolveTournamentOutboxDispatch(eventType: string): TournamentOutboxDispatch {
  if (eventType === TOURNAMENT_EVENT_DEPOSIT_CONFIRMED) return "deposit_confirmed";
  if (eventType === TOURNAMENT_EVENT_ACTION_RECORDED) return "action_recorded";
  if (eventType === TOURNAMENT_EVENT_BLOCK_MINED) return "block_mined";
  return null;
}

export function tournamentOutboxRetryDecision(
  retryCount: number,
  maxRetries = MAX_RETRIES,
): { status: "pending"; exhausted: boolean } {
  return { status: "pending", exhausted: retryCount >= maxRetries };
}

export function isTournamentOutboxLeaseExpired(
  updatedAt: Date,
  now: Date,
  ...
  for (const row of pending) {
    const leaseHeartbeat = setInterval(() => {
      void prisma.tournamentDomainOutbox
        .updateMany({
          where: { id: row.id as never, status: "processing" },
    ...
    try {
      ...
      if (dispatch === "deposit_confirmed") {
        ...
        await handleMiningBlockSettled(row.payload as MiningBlockSettledPayload);
      }
