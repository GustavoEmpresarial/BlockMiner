...
function tournamentUpperBound(tournament) {
    const now = new Date();
...
async function sumContributionsByUser(tournamentId) {
 ...
 */
async function expectedScoresFromSources(tournament) {
 ...
 */
export async function detectOfferwallDrift(tournament) {
...
export async function listRecentDriftAlerts(tournamentId, limit = 50) {
    try {
...
export async function listShadowValidationAlerts(_tournamentId, _limit = 50) {
    return [];
}
export async function runOfferwallShadowValidation() {
    return [];
}
