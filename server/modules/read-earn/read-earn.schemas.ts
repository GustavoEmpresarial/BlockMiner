/** Ported from legacy/server/utils/readEarnSchemas.ts. */
import { z } from "zod";
import { READ_EARN_BLK, READ_EARN_HASHRATE, READ_EARN_MACHINE } from "./read-earn.errors.js";
...
const baseFields = {
  ...
  partnerUrl: z.string().trim().min(1).max(2048).refine(isHttpUrl, "partnerUrl must be a valid http(s) URL"),
  rewardType: z.string().transform((s) => s.toLowerCase()).pipe(rewardTypeZ),
...
export const readEarnAdminCreateSchema = z
  .object({ ...baseFields, rewardCode: z.string().min(6).max(128) })
  .superRefine((data, ctx) => {
    ...
    if (data.rewardType === READ_EARN_MACHINE && !data.rewardMinerId) {
      ctx.addIssue({ code: "custom", path: ["rewardMinerId"], message: "rewardMinerId is required when rewardType is machine" });
    }
  });
...
export const readEarnAdminUpdateSchema = z
  .object({
    ...
    partnerUrl: z.string().trim().min(1).max(2048).refine(isHttpUrl, "partnerUrl must be a valid http(s) URL").optional(),
    rewardType: z.string().transform((s) => s.toLowerCase()).pipe(rewardTypeZ).optional(),
...
export function parseReadEarnCreate(body: unknown) {
  return readEarnAdminCreateSchema.parse(body);
}
export function parseReadEarnUpdate(body: unknown) {
  return readEarnAdminUpdateSchema.parse(body);
}
export function parseReadEarnRedeem(body: unknown) {
