...
export type GameFinishDenyCode = keyof typeof GAME_FINISH_DENY_MESSAGES;

export function gameFinishDenyMessage(code: string, locale = "pt"): string {
  const entry = GAME_FINISH_DENY_MESSAGES[code as GameFinishDenyCode];
  if (!entry) return GAME_FINISH_DENY_MESSAGES.antibot_blocked.pt;
  const lang = String(locale || "pt").toLowerCase();
  if (lang.startsWith("en")) return entry.en;
  if (lang.startsWith("es")) return entry.es;
  return entry.pt;
}

export function isSecurityRejectCode(code: string | null | undefined): boolean {
  const c = String(code || "");
  return (
    c === "anti_cheat_timing" ||
    c === "anti_cheat_burst" ||
