...
import { tryAcquireUserGameSession, releaseUserGameSession, releaseAllUserGameSessionsForSocket, clearStaleUserGameSession, } from "./games.session-lock.js";
...
import { secureShuffle, generateStableBoard, findMatches, processCascades } from "./games.match3.pure.js";
import { readMatch3GridCoord, readClampedNumber, skyMinElapsedMsForPipes, } from "./games.pure.js";
import { CART_LANES, CART_MAX_HEALTH, CART_TARGET_SCORE, CART_TIME_LIMIT_SECONDS, CART_TICK_MS, CART_BASE_SPEED, cartDifficultyFactor, cartRoadSpeed, cartRushScore, advanceCartRushEvents, resolveCartRushCollisions, advanceCartRushSpawn, } from "./games.cartrush.pure.js";
...
function getMemoryBoard(s) {
    const b = s.board;
    return Array.isArray(b) ? b : [];
}
function getMemoryFlipped(s) {
    const f = s.flipped;
...
export function registerGamesSocketHandlers(io) {
    io.on("connection", (socket) => {
        socket.on("game:start", async (gameSlug) => {
            try {
                ...
                if (!userId)
                    return socket.emit("game:error", { code: "invalid_session" });
        ...
        socket.on("game:action", (action) => {
            ...
            if (state.slug === "crypto-memory" && a.type === "flip") {
                ...
                if (!Array.isArray(state.flipped))
                    state.flipped = [];
                const flipped = getMemoryFlipped(state);
                if (flipped.length >= 2)
                    return;
                const cardId = Number(a.cardId);
                if (!Number.isInteger(cardId) || cardId < 0 || cardId >= board.length)
                    return;
                ...
                if (!Number.isInteger(lane) || lane < 0 || lane >= CART_LANES)
                    return;
                state.lane = lane;
                ...
                if (a.type === "flap") {
                    ...
                    handleSkyRunnerCheckpoint(io, socket, state, action);
                }
                else if (a.type === "finish") {
                    handleSkyRunnerFinish(io, socket, state, action);
                }
            }
        });
        socket.on("game:end", () => {
...
function handleMatch3Swap(io, socket, state, from, to) {
...
function handleSkyRunnerFlap(state) {
    const now = Date.now();
    if (now - (state.lastFlapAt || 0) < SKY_MIN_FLAP_INTERVAL_MS)
        return;
    state.lastFlapAt = now;
}
function handleSkyRunnerCheckpoint(io, socket, state, action) {
    ...
    if (pipesPassed === null || elapsedMs === null || lives === null || score === null)
        return;
    if (pipesPassed < (Number(state.pipesPassed) || 0))
        return;
...
function handleSkyRunnerFinish(io, socket, state, action) {
    const pipesPassed = readClampedNumber(action.pipesPassed, 0, SKY_TARGET_PIPES);
    const elapsedMs = readClampedNumber(action.elapsedMs, 0, 30 * 60 * 1000);
    const score = readClampedNumber(action.score, 0, SKY_TARGET_PIPES * 1000);
...
function tickCartRush(io, socket, state) {
    const live = GAME_SESSIONS.get(socket.id);
    if (!live || live !== state || state.isFinished || state.slug !== "cart-rush")
        return;
...
async function finishGame(io, socket, state, success, failureCode = "session_ended") {
    if (state.isFinished)
        return;
    clearMemoryMismatchTimer(state);
    ...
    if (!success) {
        ...
        const earlyCheat = failureCode === "sky_cheat_timing" ||
        ...
        const message = earlyCheat || isSecurityRejectCode(messageCode)
    ...
    const trust = gameSlug === "cart-rush"
    ...
    if (!antibotGate.allowed) {
        logger.warn(`[AntibotGate] Denied userId=${userId} game=${gameSlug} reason=${antibotGate.reason} score=${antibotGate.riskScore} band=${antibotGate.band}`);
        prisma.gameSessionLog
            .create({
            data: {
                userId,
    ...
    const turnstileGate = await assertGameTurnstileForReward(prisma, userId, state.cfTurnstileToken, ip || undefined);
    if (!turnstileGate.ok) {
