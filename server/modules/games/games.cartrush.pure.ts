...
export type CartEventKind = "coin" | "cone" | "barrier" | "pothole" | "enemy-car";

export type CartRushEvent = {
  id: string;
  lane: number;
  kind: CartEventKind;
  progress: number;
  speed: number;
  variant: (typeof CART_ENEMY_VARIANTS)[number];
  checked?: boolean;
};

/** Ramps 0 → 1 over CART_DIFFICULTY_RAMP_MS of elapsed play time. */
export function cartDifficultyFactor(elapsedMs: number): number {
 ...
 */
export function createCartEvent(distance: number, difficulty: number): CartRushEvent {
  const rand = Math.random();
  const variant = CART_ENEMY_VARIANTS[crypto.randomInt(0, CART_ENEMY_VARIANTS.length)];
  let kind: CartEventKind;
 ...
 */
export function advanceCartRushEvents(events: CartRushEvent[], roadSpeed: number): CartRushEvent[] {
...
export type CartRushCollisionResult = {
  survivors: CartRushEvent[];
  hit: CartRushEvent | null;
  healthDelta: number;
  btcDelta: number;
};
 ...
 */
export function resolveCartRushCollisions(events: CartRushEvent[], lane: number): CartRushCollisionResult {
  let hit: CartRushEvent | null = null;
  let healthDelta = 0;
  let btcDelta = 0;
  const survivors: CartRushEvent[] = [];
...
export function cartRoadSpeed(difficulty: number): number {
...
export function cartRushScore(distance: number, btcCount: number): number {
 ...
 */
export function advanceCartRushSpawn(
  events: CartRushEvent[],
  spawnCooldownMs: number,
  distance: number,
  difficulty: number,
): { events: CartRushEvent[]; spawnCooldownMs: number } {
  ...
  if (cooldown < CART_MIN_SPAWN_MS) cooldown = CART_MIN_SPAWN_MS;
