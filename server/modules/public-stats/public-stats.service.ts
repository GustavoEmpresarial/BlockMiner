...
/**
 * Ported from legacy/server/services/publicLiveStatsService.ts, flattened into the
 * public-stats module (current/ doctrine keeps no global services/ folder — the
 * only consumer of this service is public-stats.controller.ts).
 */
...
export async function getPublicLiveStats() {
    ...
    const [usersTotal, polUserBalances, depositsAll, withdrawalsAll, deposits24h, withdrawals24h, pendingWithdrawals, newUsers24h, activeMiners, totalTransactions, transactionLast24h, blkConfig, recentDistributions,] = await Promise.all([
        prisma.user.count(),
