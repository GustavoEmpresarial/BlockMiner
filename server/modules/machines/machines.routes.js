...
machinesRouter.post("/toggle", requireAuth, machinesLimiter, requireCriticalIdempotency({ scope: "machine_toggle" }), machinesController.toggleMachine);
machinesRouter.post("/remove", requireAuth, machinesLimiter, requireCriticalIdempotency({ scope: "machine_remove" }), machinesController.removeMachine);
machinesRouter.post("/move", requireAuth, machinesLimiter, requireCriticalIdempotency({ scope: "machine_move" }), machinesController.moveMachine);
