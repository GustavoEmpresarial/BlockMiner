...
const GENERIC_LABELS = new Set(["máquina", "maquina", "máquina custom", "maquina custom", "miner", "unknown miner", "gpu 1 ghs"]);
const LOCATIONS = new Set(["RACK", "INVENTORY", "WAREHOUSE"]);
function emptyRelink(minerName, message) {
    return {
        ok: false,
        minerName,
        catalogMinerId: null,
...
export async function listOrphanMachineTypes(prisma, limit = 60) {
...
export async function relinkOrphanMachineTypeToCatalog(prisma, minerName) {
    const label = String(minerName ?? "").trim();
    if (!label)
        return emptyRelink(label, "Miner name is required.");
    ...
    if (!catalog)
        return emptyRelink(label, "No catalog miner has this name.");
    const where = { minerId: null, eventMinerId: null, minerName: label };
...
export async function listBrokenMachineGroups(prisma) {
    const rows = await prisma.$queryRaw `
  ...
  `;
    ...
    const catalogByRate = new Map();
    const eventByRate = new Map();
    for (const miner of catalogMiners)
        catalogByRate.set(miner.baseHashRate, [...(catalogByRate.get(miner.baseHashRate) ?? []), miner]);
    for (const miner of eventMiners)
        eventByRate.set(miner.hashRate, [...(eventByRate.get(miner.hashRate) ?? []), miner]);
    const knownEventNames = new Set(eventMiners.map((miner) => miner.name.trim().toLowerCase()));
    return rows.flatMap((row) => {
        if (!LOCATIONS.has(row.location))
            return [];
        const minerName = String(row.miner_name ?? "");
        const eventName = parseEventMinerDisplayName(minerName);
        if (eventName && knownEventNames.has(eventName.toLowerCase()))
            return [];
        ...
        return [{
                minerName, hashRate: Number(row.hash_rate), location: row.location,
...
async function findOwnedIds(prisma, minerName, hashRate, location) {
...
export async function assignCatalogMinerToBrokenGroup(prisma, input) {
    ...
    if (!catalog)
        return { ok: false, assigned: 0, message: "Catalog miner not found." };
    if (!ownedIds.length || !validLocation(input.location))
        return { ok: false, assigned: 0, message: "No machines match this group." };
    await prisma.$transaction(async (tx) => {
        await tx.userOwnedMachine.updateMany({ where: { id: { in: ownedIds } }, data: { minerId: catalog.id, minerName: catalog.name } });
        if (input.location === "RACK")
            await tx.userMiner.updateMany({ where: { ownedMachineId: { in: ownedIds } }, data: { minerId: catalog.id } });
        if (input.location === "INVENTORY")
            await tx.userInventory.updateMany({ where: { ownedMachineId: { in: ownedIds } }, data: { minerId: catalog.id, minerName: catalog.name } });
        if (input.location === "WAREHOUSE")
            await tx.userVault.updateMany({ where: { ownedMachineId: { in: ownedIds } }, data: { minerId: catalog.id, minerName: catalog.name } });
    });
    return { ok: true, assigned: ownedIds.length, message: `${ownedIds.length} machine(s) assigned to ${catalog.name}.` };
}
export async function assignEventMinerToBrokenGroup(prisma, input) {
    ...
    if (!event)
        return { ok: false, assigned: 0, message: "Event miner not found." };
    if (!ownedIds.length || !validLocation(input.location))
        return { ok: false, assigned: 0, message: "No machines match this group." };
    ...
    await prisma.$transaction(async (tx) => {
        ...
        if (input.location === "RACK")
            await tx.userMiner.updateMany({ where: { ownedMachineId: { in: ownedIds } }, data: { imageUrl } });
        if (input.location === "INVENTORY")
            await tx.userInventory.updateMany({ where: { ownedMachineId: { in: ownedIds } }, data: { minerName, imageUrl } });
        if (input.location === "WAREHOUSE")
            await tx.userVault.updateMany({ where: { ownedMachineId: { in: ownedIds } }, data: { minerName, imageUrl } });
    });
    return { ok: true, assigned: ownedIds.length, message: `${ownedIds.length} machine(s) assigned to event ${event.name}.` };
}
export async function autoAssignBrokenMachines(_prisma) {
    return {
        ok: false,
        assigned: 0,
        skipped: 0,
