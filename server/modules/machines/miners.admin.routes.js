...
import { assignCatalogMinerToBrokenGroup, assignEventMinerToBrokenGroup, autoAssignBrokenMachines, listBrokenMachineGroups, listOrphanMachineTypes, relinkOrphanMachineTypeToCatalog, } from "./miners.admin.repair.js";
...
minersAdminRouter.get("/miners", async (req, res) => {
    try {
...
minersAdminRouter.get("/miners/orphan-types", async (_req, res) => {
    try {
...
minersAdminRouter.post("/miners/orphan-types/relink", async (req, res) => {
    try {
...
minersAdminRouter.get("/miners/broken-machines", async (_req, res) => {
    try {
...
minersAdminRouter.post("/miners/broken-machines/assign", async (req, res) => {
    try {
        const body = (req.body ?? {});
...
minersAdminRouter.post("/miners/broken-machines/auto-assign", async (req, res) => {
    try {
...
minersAdminRouter.get("/miners/:id", async (req, res) => {
    try {
...
minersAdminRouter.post("/miners", async (req, res) => {
    try {
        const body = (req.body ?? {});
...
minersAdminRouter.patch("/miners/:id", async (req, res) => {
    try {
        const id = Number(req.params.id);
        if (!Number.isSafeInteger(id) || id < 1) {
            res.status(400).json({ ok: false, message: "Invalid id." });
        ...
        for (const key of ["baseHashRate", "price", "slotSize", "sortOrder"]) {
            if (body[key] != null && Number.isFinite(Number(body[key])))
                data[key] = Number(body[key]);
        }
        for (const key of ["isActive", "showInShop", "isArchived"]) {
...
minersAdminRouter.post("/miners/:id/toggle-active", async (req, res) => {
    try {
        const id = Number(req.params.id);
        if (!Number.isSafeInteger(id) || id < 1) {
            res.status(400).json({ ok: false, message: "Invalid id." });
...
minersAdminRouter.post("/miners/:id/toggle-store", async (req, res) => {
    try {
        const id = Number(req.params.id);
        if (!Number.isSafeInteger(id) || id < 1) {
            res.status(400).json({ ok: false, message: "Invalid id." });
