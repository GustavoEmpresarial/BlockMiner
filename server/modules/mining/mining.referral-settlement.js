 ...
 */
export function mergeSettlementBalanceDeltas(minerRewards, referrerByMinerId, commissionRate) {
    const deltas = new Map();
    const bump = (userId, pol, shib, lifetime) => {
        if (pol <= 0 && shib <= 0)
            return;
...
export function buildReferralEarningsRows(minerRewards, referrerByMinerId, blockNumber, createdAt, commissionRate) {
    const rows = [];
    for (const reward of minerRewards) {
        const referrer = referrerByMinerId.get(reward.userId);
        const pol = reward.rewardAmount;
        const shib = reward.rewardAmountShib ?? 0;
        if (!referrer || referrer === reward.userId || (pol <= 0 && shib <= 0))
            continue;
        rows.push({
            referrerId: referrer,
