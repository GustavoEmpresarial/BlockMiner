...
import { ALLOCATION_BPS_MAX, readMiningBlockRewardPol, readMiningBlockRewardShib, normalizeAllocationBps, readMiningBlockDurationMs, nextAlignedBoundary, isDuplicateBlockError, readBoundedIntegerEnv, } from "./mining.config.js";
const logger = rootLogger.child("MiningEngine");
export { ALLOCATION_BPS_MAX, readMiningBlockRewardPol, readMiningBlockRewardShib, normalizeAllocationBps, readMiningBlockDurationMs, nextAlignedBoundary, };
...
export class MiningEngine {
    ...
    blockHistory;
    leaderboardCache;
    leaderboardCacheDirty;
    logRewardCallback;
    persistBlockRewardsCallback;
        ...
        if (this.profileLoader) {
            ...
            if (profile) {
                ...
                if (miner) {
                    ...
                    if (rawAlloc != null)
                        miner.miningAllocationPolBps = normalizeAllocationBps(rawAlloc);
                    const rawShib = p.shib_balance ?? p.shibBalance;
                    if (rawShib != null)
                        miner.shibBalance = Number(rawShib) || 0;
                    // Sincroniza saldo:
    ...
    createOrGetMiner({ userId, username, walletAddress, profile }) {
        ...
        if (existing) {
            ...
            if (profile) {
                ...
                if (rawAlloc != null)
                    existing.miningAllocationPolBps = normalizeAllocationBps(rawAlloc);
                const rawShib = profile.shib_balance ?? profile.shibBalance;
                if (rawShib != null)
                    existing.shibBalance = Number(rawShib) || 0;
        ...
        const miner = {
            ...
            miningPayoutMode: profile?.mining_payout_mode === "blk" || profile?.miningPayoutMode === "blk" ? "blk" : "pol",
            miningAllocationPolBps: normalizeAllocationBps(profile?.mining_allocation_pol_bps ?? profile?.miningAllocationPolBps ?? ALLOCATION_BPS_MAX),
            lifetimeMinedShib: 0,
            lastShibReward: 0,
     ...
     */
    setMinerAllocation(minerId, rawBps) {
        const miner = this.miners.get(minerId);
        if (!miner)
    ...
    distributeRewards() {
        ...
        for (const minerId of roundSnapshot.keys()) {
            if (this.miners.has(minerId))
                ...
                this.roundWork.delete(minerId); // miner gone: drop the key instead of leaking it
        }
        const blockReward = this.rewardBase;
        const blockRewardShib = this.rewardBaseShib;
        const splitByMiner = new Map();
        let totalWorkPol = 0;
        let totalWorkShib = 0;
    ...
    async drainSettlements() {
        for (;;) {
            ...
            await pending;
            if (pending === this._settleChain)
                return;
        }
    }
    /** Writes one settled block, retrying until it lands or the budget runs out. */
    async persistSettlement(payload) {
        if (!this.persistBlockRewardsCallback)
            return;
    ...
    finalizeBlockDistribution(_num, _reward) {
        this.blockNumber += 1;
        this.blockProgress = 0;
        this.lastBlockAt = Date.now();
