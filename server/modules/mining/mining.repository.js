...
import { REFERRAL_MINING_COMMISSION_RATE, mergeSettlementBalanceDeltas, buildReferralEarningsRows, } from "./mining.referral-settlement.js";
 ...
 */
async function computeBaseHashRate(userId) {
    const now = new Date();
...
export async function getOrCreateMinerProfile(userId) {
...
export async function updateUserMiningAllocation(userId, polBps) {
    await prisma.user.update({
        where: { id: userId },
        data: { miningAllocationPolBps: polBps },
    });
}
export async function updateUserMiningPayoutMode(userId, mode) {
    await prisma.user.update({
        where: { id: userId },
 ...
 */
export async function persistMinerBalanceDelta(userId, balance, lastPersistedBalance) {
    const pendingDelta = balance - lastPersistedBalance;
    if (Math.abs(pendingDelta) < 0.0000001)
        return lastPersistedBalance;
 ...
 */
export async function persistBlockRewards(payload) {
    ...
    const balanceRows = Array.from(balanceDeltas.entries())
        ...
        .map(([userId, d]) => Prisma.sql `(${userId}::int, ${d.polDelta}::double precision, ${d.shibDelta}::double precision, ${d.lifetimeMinedDelta}::double precision)`);
    const referralEarningsRows = buildReferralEarningsRows(sortedRewards, referrerByMinerId, blockNumber, timestamp, REFERRAL_MINING_COMMISSION_RATE).sort((a, b) => a.referrerId - b.referrerId || a.referredId - b.referredId);
...
export async function loadMaxBlockNumber() {
