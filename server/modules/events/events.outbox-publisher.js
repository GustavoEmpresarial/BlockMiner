...
import { isKafkaEnabled, readEventOutboxBatchSize, readEventOutboxPublishCronMs, } from "./events.config.js";
...
export async function publishOutboxBatchOnce() {
    if (!isKafkaEnabled())
    ...
    if (!producer)
        return { published: 0, skipped: rows.length };
    const okIds = [];
    for (const row of rows) {
        try {
            await producer.send({
                ...
                messages: [
                    {
                        key: String(row.payloadJson?.userId ?? row.eventId),
...
export function startEventOutboxPublisherCron() {
