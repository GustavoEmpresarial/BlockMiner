...
export function isKafkaEnabled(raw = process.env[KAFKA_ENABLED_ENV_KEY]) {
...
export function readKafkaBrokers(raw = process.env[KAFKA_BROKERS_ENV_KEY]) {
...
export function readKafkaClientId(raw = process.env[KAFKA_CLIENT_ID_ENV_KEY]) {
    if (raw == null || String(raw).trim() === "")
        return DEFAULT_KAFKA_CLIENT_ID;
    return String(raw).trim();
}
export function readKafkaTopicEarnings(raw = process.env[KAFKA_TOPIC_EARNINGS_ENV_KEY]) {
    if (raw == null || String(raw).trim() === "")
        return DEFAULT_KAFKA_TOPIC_EARNINGS;
    return String(raw).trim();
}
export function readKafkaTopicPower(raw = process.env[KAFKA_TOPIC_POWER_ENV_KEY]) {
    if (raw == null || String(raw).trim() === "")
        return DEFAULT_KAFKA_TOPIC_POWER;
    return String(raw).trim();
}
export function readKafkaStatsConsumerGroup(raw = process.env[KAFKA_STATS_CONSUMER_GROUP_ENV_KEY]) {
    if (raw == null || String(raw).trim() === "")
        return DEFAULT_KAFKA_STATS_CONSUMER_GROUP;
    return String(raw).trim();
}
export function readEventOutboxPublishCronMs(raw = process.env[EVENT_OUTBOX_PUBLISH_CRON_MS_ENV_KEY]) {
...
export function readEventOutboxBatchSize(raw = process.env[EVENT_OUTBOX_BATCH_SIZE_ENV_KEY]) {
    const n = Number(raw ?? DEFAULT_EVENT_OUTBOX_BATCH_SIZE);
    return Number.isInteger(n) && n > 0 ? n : DEFAULT_EVENT_OUTBOX_BATCH_SIZE;
}
export function readStatsMaterializationMaxLagMs(raw = process.env[STATS_MATERIALIZATION_MAX_LAG_MS_ENV_KEY]) {
