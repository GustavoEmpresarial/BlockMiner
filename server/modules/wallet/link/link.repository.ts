/**
 * Wallet-link persistence — challenges are stored in the generic `callback_queue`
 * table (same reuse pattern as legacy, avoids a dedicated table for a short-lived
 * nonce). The linked address itself lives on `User.walletAddress`.
 */
...
export async function invalidateWalletLinkChallenges(userId: number, addressLower: string): Promise<void> {
  ...
  await prisma.callbackQueue.updateMany({
    where: { callbackType: WALLET_LINK_CALLBACK_TYPE, userId, callbackHash: hash, status: "pending" },
...
export async function createWalletLinkChallenge(
  ...
  const row = await prisma.callbackQueue.create({
    data: {
      ...
      status: "pending",
      data: { address, addressLower, chainId, message, nonce, expiresAtMs },
    },
  });
  return row.id;
}
...
export async function findValidWalletLinkChallenge(
  userId: number,
  addressLower: string,
): Promise<{ id: number; message: string; nonce: string; address: string; chainId: number } | null> {
  const hash = linkChallengeHash(userId, addressLower);
...
export async function markWalletLinkChallengeUsed(challengeId: number): Promise<void> {
  await prisma.callbackQueue.update({ where: { id: challengeId }, data: { status: "completed", processedAt: new Date() } });
}
