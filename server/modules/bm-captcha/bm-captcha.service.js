...
import { BM_CAPTCHA_PURPOSE_OFFERWALL_EXTERNAL, bmCaptchaChallengeTtlMs, bmCaptchaEnabled, bmCaptchaHitRadiusPx, bmCaptchaHmacSecret, bmCaptchaMaxAttempts, bmCaptchaMinClickGapMs, bmCaptchaMinSolveMs, bmCaptchaMintMaxPerWindow, bmCaptchaMintWindowMs, bmCaptchaPassTtlMs, isBmCaptchaProvider, } from "./bm-captcha.config.js";
 ...
 */
function clicksMatchTargets(clicks, targetIds, minGapMs) {
...
export async function mintChallenge(input) {
    ...
    const quota = await store.tryConsumeMintQuota(input.userId, bmCaptchaMintMaxPerWindow(), bmCaptchaMintWindowMs());
    if (!quota) {
...
export async function verifyChallenge(userId, body) {
    const blocked = assertCaptchaReady();
    if (blocked)
        return blocked;
    const challengeId = String(body.challengeId || "").trim();
    if (!challengeId)
        return { ok: false, code: "MISSING_CHALLENGE", status: 400 };
