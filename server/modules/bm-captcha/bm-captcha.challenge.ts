...
import type {
  ...
  BmCaptchaChallengeSecret,
  BmCaptchaPurpose,
  BmCaptchaSpritePublic,
...
export function glyphFromSeed(glyphSeed: string): { hue: number; shape: number; rot: number } {
...
export function mintChallengePair(input: {
  ...
  now?: Date;
}): { public: BmCaptchaChallengePublic; secret: BmCaptchaChallengeSecret } {
  ...
  const sprites: BmCaptchaSpritePublic[] = [];
  ...
  for (let i = 0; i < total; i++) {
    ...
    const spr: BmCaptchaSpritePublic = {
      id: i,
  ...
  const secret: BmCaptchaChallengeSecret = {
    challengeId,
    userId: input.userId,
  ...
  const pub: BmCaptchaChallengePublic = {
    kind: "canvas_click",
    challengeId,
    purpose: input.purpose,
    provider: input.provider,
