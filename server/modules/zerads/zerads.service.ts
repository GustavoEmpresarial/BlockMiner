...
import {
  loadAndEvaluateZeradsAntibotGate,
  trimZeradsClicksForSecurity,
  zeradsVelocityWindowBounds,
} from "./zerads.security.js";
import type { ZeradsCallbackParams, ZeradsCallbackResult } from "./zerads.types.js";
...
export const ZERADS_ALLOWED_IPS = new Set(
 ...
 */
export function buildCallbackHash(username: string, amountZer: number, clicks: number, now: Date = new Date()): string {
  const bucket = Math.floor(now.getTime() / 300_000); // 5-min window
  const payload = `${username}|${amountZer}|${clicks}|${bucket}`;
  return crypto.createHash("sha256").update(payload).digest("hex");
}

function isUniqueCallbackHashViolation(err: unknown): boolean {
  if (
    err instanceof Prisma.PrismaClientKnownRequestError &&
    err.code === "P2002" &&
    JSON.stringify((err.meta as { target?: unknown } | undefined)?.target ?? "").includes("callback_hash")
  ) {
    return true;
  }
 ...
 */
export async function processZeradsCallback(params: ZeradsCallbackParams): Promise<ZeradsCallbackResult> {
...
export async function getUserZeradsLink(userId: number) {
  const user = await zeradsRepo.findUsernameById(userId);
  if (!user?.username) return { ok: false as const, reason: "no_username" as const };
...
export async function getZeradsHistoryForUser(userId: number, page: number) {
...
export async function getZeradsStatsForUser(userId: number) {
