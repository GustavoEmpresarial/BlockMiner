/**
 * Ported from legacy/server/modules/zerads/zerads.controller.ts.
 *
 * The callback handler intentionally does NOT follow the module's usual
 * `{ ok: true/false, ... }` JSON response doctrine (see ARQUITETURA.md) — it's a public
 * S2S callback hit directly by the Zerads provider, which expects PHP-style plain text
 * `"1"` (success) / `"0"` (failure). Preserving the exact response format is required
 * for the external integration to keep working; this is a deliberate, documented exception.
 */
