...
export const ZERADS_MAX_CLICKS_PER_UTC_DAY = Math.max(1, parseInt(process.env.ZERADS_MAX_CLICKS_PER_UTC_DAY ?? process.env.ZERADS_MAX_CLICKS_PER_BRT_DAY ?? "100", 10) || 100);
/** UTC-day [start, end) bounds for the callback's `now`, using the site-wide UTC calendar util. */
export function zeradsUtcDayBounds(now) {
    const dayKey = getUtcDayKey(now);
    return { start: getUtcPeriodStartAt(dayKey), end: getUtcPeriodResetAt(dayKey) };
}
export function capZeradsClicksForUtcDay(clicks) {
...
export function aggregateZeradsClicksPerUser(rows) {
