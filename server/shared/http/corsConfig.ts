 ...
 * Env `CORS_ORIGINS` (comma-separated) is merged with partner frontends below.
 */
...
export function parseCorsOriginsList(): string[] {
...
export function buildSocketIoCorsOptions(): CorsOptions {
  assertProductionCorsConfigured();
  const origins = parseCorsOriginsList();
  return {
