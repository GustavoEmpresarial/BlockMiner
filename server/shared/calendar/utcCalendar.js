 ...
 */
export function getUtcIsoWeekKey(date = new Date()) {
 ...
 */
export function normalizeUtcDayKey(input) {
    if (input instanceof Date)
        return getUtcDayKey(input);
    ...
    if (ymdDash)
        return buildDayKey(ymdDash[1], ymdDash[2], ymdDash[3]);
    const ymdSlash = raw.match(/^(\d{4})\/(\d{1,2})\/(\d{1,2})(?:[T\s].*)?$/);
    if (ymdSlash)
        return buildDayKey(ymdSlash[1], ymdSlash[2], ymdSlash[3]);
    const mdySlash = raw.match(/^(\d{1,2})\/(\d{1,2})\/(\d{4})(?:[T\s].*)?$/);
    if (mdySlash)
        return buildDayKey(mdySlash[3], mdySlash[1], mdySlash[2]);
    const parsed = new Date(raw);
    if (!Number.isNaN(parsed.getTime()))
        return getUtcDayKey(parsed);
    return "";
}
/** Textual aliases that have appeared for the same day (legacy data-shape drift). */
export function getUtcDayKeyAliases(input = new Date()) {
...
export function addDaysToUtcDayKey(dateKey, deltaDays) {
    const normalized = normalizeUtcDayKey(dateKey);
    if (!normalized)
        throw new Error(`Invalid UTC date key: ${String(dateKey)}`);
...
export function getUtcMonthPeriodKey(date = new Date()) {
    return getUtcDayKey(date).slice(0, 7);
}
export function isSameUtcDay(storedKey, currentKey) {
...
export function getUtcDayKeyLookupKeys(dayKey) {
...
export function getUtcPeriodResetAt(dayKey) {
    const normalized = normalizeUtcDayKey(dayKey);
    if (!normalized)
        throw new Error(`Invalid UTC date key: ${dayKey}`);
    const [y, m, d] = normalized.split("-").map(Number);
    return new Date(Date.UTC(y, m - 1, d + 1, 0, 0, 0));
}
/** Start instant of this UTC day (the UTC midnight it begins at). */
export function getUtcPeriodStartAt(dayKey) {
    const normalized = normalizeUtcDayKey(dayKey);
    if (!normalized)
        throw new Error(`Invalid UTC date key: ${dayKey}`);
    const [y, m, d] = normalized.split("-").map(Number);
