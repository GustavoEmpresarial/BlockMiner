...
import { featureTurnstileDenyBody, getFeatureTurnstileStatus, submitFeatureTurnstilePass, } from "./featureTurnstileGate.js";
...
const passBodySchema = z
    ...
    .strict();
export function featureTurnstileStatusHandler(purpose) {
...
export function featureTurnstilePassHandler(purpose) {
    return async (req, res) => {
        try {
            const user = requireSessionUser(req, res);
            if (!user)
            ...
            const result = await submitFeatureTurnstilePass(purpose, user.id, parsed.data.cfTurnstileToken, typeof req.ip === "string" ? req.ip : undefined);
            if (!result.ok) {
