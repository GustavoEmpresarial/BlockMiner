/**
 * Opt-in auth diagnostics (AUTH_DEBUG_LOG=1) plus automatic tracing for partner
 * iframe / cross-origin login attempts (genesisdao.tech, minercore.online, etc.).
 * Never logs passwords, OTP codes, or full identifiers — only domain hints and flow metadata.
 */
...
function tryHostnameFromUrl(raw: string): string | undefined {
...
function identifierHints(identifier: unknown): Record<string, unknown> {
  const value = String(identifier ?? "").trim();
  if (!value) return { identifierPresent: false };
...
export function buildAuthRequestContext(req: Request, extra: Record<string, unknown> = {}): Record<string, unknown> {
...
function shouldTrace(ctx: Record<string, unknown>): boolean {
...
export function authLoginTrace(
  event: string,
  req: Request,
