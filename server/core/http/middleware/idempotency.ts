...
export type CriticalIdempotencyOptions = { scope: string };
...
function stableRequestHash(input: { body: unknown; params: unknown; path: string }): string {
  const body =
    typeof input.body === "object" && input.body !== null
      ? { ...(input.body as Record<string, unknown>) }
      : input.body;
  if (body && typeof body === "object") {
    for (const k of EPHEMERAL_BODY_KEYS) delete (body as Record<string, unknown>)[k];
  }
  const normalized = JSON.stringify(sortKeysDeep({ body, params: input.params, path: input.path }));
  return createHash("sha256").update(normalized).digest("hex");
}

function buildIdempotencyErrorJson(code: string, extra?: Record<string, unknown>) {
...
export function requireCriticalIdempotency(opts: CriticalIdempotencyOptions): RequestHandler {
  ...
  return function criticalIdempotency(req: Request, res: Response, next: NextFunction): void {
    const raw = req.get("Idempotency-Key") || req.get("idempotency-key") || (req.body as Record<string, unknown> | undefined)?.idempotencyKey;
...
type CallbackQueueDelegate = {
  findFirst: (args: unknown) => Promise<{ id: number; data: unknown } | null>;
  create: (args: unknown) => Promise<{ id: number }>;
  update: (args: unknown) => Promise<unknown>;
};

function callbackQueue(client: typeof prisma): CallbackQueueDelegate {
  return (client as unknown as { callbackQueue: CallbackQueueDelegate }).callbackQueue;
}

function logicalHash(scope: string, userId: number, idempotencyKey: string): string {
  return createHash("sha256").update(`${scope}|${userId}|${idempotencyKey}`, "utf8").digest("hex");
}

export type IdempotencyLease =
  | { type: "replay"; responseJson: object }
  | { type: "mismatch" }
  | { type: "busy" }
  | { type: "lease"; leaseToken: string; h: string; resultTtlSec: number; requestHash: string };

export type CriticalIdempotencyContext = { scope: string; idempotencyKey: string; requestHash: string };
export type ResolveCriticalMutationResult = { lease: IdempotencyLease & { type: "lease" }; ci: CriticalIdempotencyContext };

const CLAIM_TTL_SEC = 120;
const RESULT_TTL_SEC = 86_400;

export async function resolveCriticalMutation(req: Request, res: Response): Promise<ResolveCriticalMutationResult | null> {
  ...
  const phase = await prisma.$transaction(async (tx) => {
    ...
    const cbq = callbackQueue(tx as unknown as typeof prisma);
    ...
    if (data.phase === "done" && data.responseJson != null) {
      if (data.requestHash !== ci.requestHash) return { type: "mismatch" } as const;
      return { type: "replay", responseJson: data.responseJson as object } as const;
    }
    if (data.phase === "lease" && Number(data.leaseUntilMs ?? 0) > now) {
      if (data.requestHash != null && data.requestHash !== ci.requestHash) return { type: "mismatch" } as const;
    ...
    return { type: "lease", leaseToken, h, resultTtlSec: RESULT_TTL_SEC, requestHash: ci.requestHash } as const;
  });
...
export async function finalizeCriticalMutationSuccess(
  lease: IdempotencyLease & { type: "lease" },
...
export async function cancelCriticalMutation(lease: (IdempotencyLease & { type: "lease" }) | null | undefined): Promise<void> {
  if (!lease) return;
  const cbq = callbackQueue(prisma);
  const row = await cbq.findFirst({ where: { callbackType: "SEC_IDEM", callbackHash: lease.h } });
  if (row?.id) {
