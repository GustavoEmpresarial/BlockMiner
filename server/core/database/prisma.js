...
function assertNotProductionDatabase(url) {
    if (!url) {
        throw new Error("[current/server] DATABASE_URL is not set. Copy current/.env.example to current/.env " +
    ...
    if (hit) {
        throw new Error(`[current/server] Refusing to start: DATABASE_URL contains a known production identifier ("${hit}"). ` +
...
const statementTimeoutMs = Number.parseInt(String(process.env.PG_STATEMENT_TIMEOUT_MS || "30000").trim(), 10) || 30000;
const idleInTxTimeoutMs = Number.parseInt(String(process.env.PG_IDLE_IN_TX_TIMEOUT_MS || "30000").trim(), 10) || 30000;
const pool = new pg.Pool({
    connectionString,
    max: poolMaxConnections(),
    idleTimeoutMillis: Number.parseInt(String(process.env.PG_POOL_IDLE_MS || "30000").trim(), 10) || 30000,
    connectionTimeoutMillis: Number.parseInt(String(process.env.PG_POOL_CONNECTION_TIMEOUT_MS || "10000").trim(), 10) || 10000,
